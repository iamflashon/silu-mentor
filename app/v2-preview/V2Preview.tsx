...
export default function V2Preview({ catalog, config }: { catalog: { teachers: Record<V2TeacherKey, Teacher> }; config: V2Config }) {
  ...
  const visible = useMemo(() => Object.values(catalog.teachers).filter((item) => config.teachers[item.key].enabled && config.teachers[item.key].brands.includes(brand)), [brand, catalog, config]);
  ...
  return <main className={`v2-shell ${angle ? "angle" : "get"}`}>
    ...
    <section className="v2-teachers"><header><div><span>ACTIVE LEARNING SPACES</span><h2>{angle ? "從作者與書籍進入解題室" : "選擇老師，開始實際訓練"}</h2></div><p>同一套核心能力，依老師、題型及品牌設定不同模組。</p></header><div className="v2-teacher-grid">{visible.map((teacher) => <article className={`teacher-${teacher.key}`} key={teacher.key}><div className="v2-teacher-mark">{teacher.name.slice(0, 1)}</div><div><small>{teacher.brandNote}</small><h3>{teacher.name}老師</h3><p>{teacher.subject}</p><dl><div><dt>已拆解書籍／文件</dt><dd>{teacher.bookCount}</dd></div><div><dt>題庫主檔</dt><dd>{teacher.questionCount.toLocaleString()}</dd></div></dl><div className="v2-module-tags">{config.teachers[teacher.key].modules.slice(0, 5).map((module) => <span key={module}>{moduleLabel[module]}</span>)}</div><a href={`/v2-preview/${teacher.key}?brand=${brand}`}>進入真實資料樣板 →</a></div></article>)}</div></section>
    <section className="v2-micro-banner"><div><span>MICRO COURSE ENGINE</span><h2>微課跟著批改結果出現</h2><p>SRT依時間軸拆解、AI摘要知識點、綁定書籍與題目；學生可在播放時截圖，連同時間碼及前後字幕一起問AI。</p></div><a href="/admin/v2-platform?tab=micro">管理微課與SRT</a></section>
  </main>;
}
