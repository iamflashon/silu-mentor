...
export function IssuePractice() {
  ...
  function captureLawSelection() {
    const text = window.getSelection()?.toString().replace(/\s+/g, " ").trim().slice(0, 500) ?? "";
    if (text.length < 2) return;
    const compact = text.replace(/\s+/g, "");
    const match = compact.match(/(?:中華民國)?(?:憲法|民法|刑法|行政程序法|行政訴訟法|民事訴訟法|刑事訴訟法|公司法|證券交易法|保險法|票據法)第\d+(?:條之\d+|之\d+條|條)(?:第\d+項)?/u);
    setSelectedLawText(text); setDetectedLawQuery(match?.[0] ?? "");
  }
  ...
  return <section className="student-issue-practice" aria-label="練爭點">
    ...
    {result && selectedLawText && <div className="smart-selection-bar"><span>已框選：{selectedLawText}</span><button type="button" onClick={() => void lookupSelectedLaw()} disabled={!detectedLawQuery} title={detectedLawQuery ? `搜尋 ${detectedLawQuery}` : "框選內容未辨識出法規名稱與條號"}>法條搜尋</button><button type="button" onClick={() => void explainLaw()}>白話解釋</button><button type="button" aria-label="關閉框選工具" onClick={() => { setSelectedLawText(""); setDetectedLawQuery(""); }}>×</button></div>}
