...
export function IssuePractice() {
  ...
  function captureLawSelection() {
    ...
    const match = text.match(/(?:中華民國)?(?:憲法|民法|刑法|行政程序法|行政訴訟法|民事訴訟法|刑事訴訟法|公司法|證券交易法|保險法|票據法)?第\d+(?:條之\d+|之\d+條|條)(?:第\d+項)?/u);
    setSelectedLawText(match?.[0] ?? "");
  }

  async function lookupSelectedLaw() {
    if (!selectedLawText) return;
    const baseQuery = selectedLawText.replace(/第\d+項$/u, "");
  ...
  async function explainLaw() {
    if (!lawLookup?.article || lawLookup.explaining) return;
    setLawLookup({ ...lawLookup, explaining: true, error: "" });
    const response = await fetch("/api/legal-explain", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ selectedText: selectedLawText, article: lawLookup.article }) });
    const data = await response.json();
    setLawLookup((current) => current ? { ...current, explaining: false, explanation: response.ok ? String(data.explanation || "") : "", error: response.ok ? "" : data.error || "白話解釋暫時無法完成。" } : current);
  }
