                ...
                <div className="essay-chat-column">
                    ...
                    <div className="essay-chat-composer-wrap">
                      ...
                      <div className="essay-chat-composer-actions">
                        {!coachStarted ? <><span>設定完成後，開始這一題的自然對話</span><button type="button" className="essay-chat-start scholar-start-button" onClick={startEssayCoach} disabled={coaching}>開始對話</button></> : <><span>你可以繼續回答，也可以請導師直接收束</span><button type="button" className="essay-summary-button" onClick={() => void askCoach("subquestion_summary")} disabled={coaching || !coachInput.trim()}>單題總結</button><button type="button" className="essay-end-summary-button" onClick={() => void askCoach("end_summary")} disabled={coaching}>直接總結並結束</button><button type="button" className="scholar-follow-up-button" onClick={() => void generateScholarFollowUp()} disabled={coaching}>{selectedCoachMessageIndex === null ? "送出訊息" : "回覆此訊息"}</button></>}
                      </div>
