      ...
      {(activeTab === "books" || activeTab === "courses") && <section className="resource-learning-hub" aria-label={activeTab === "books" ? "書籍學習" : "影音課程學習"}>
        ...
        <div className="resource-learning-layout"><aside className={`resource-list ${activeTab === "books" ? "book-resource-list" : ""}`} aria-label="可學習資源">{activeTab === "books" ? bookResources.map((resource) => {
          ...
          return <div className={`book-resource-card ${isSelected ? "active" : ""}`} key={resource.id}>
            ...
            {isExpanded && <div className="book-resource-chapters" aria-label={`${resource.title}章節`}><div className="book-chapter-heading"><strong>本書章節</strong><span>{bookChaptersLoading && isSelected ? "正在讀取…" : bookChapters.length && isSelected ? `${bookChapters.length} 章` : isSelected && resource.documentStatus === "completed" ? "教材已索引" : "等待教材索引"}</span></div>{!isSelected ? <div className="book-chapter-empty">選取這本書後載入章節。</div> : bookChaptersLoading ? <div className="book-chapter-empty">正在從教材索引整理真實章節…</div> : bookChapters.length ? bookChapters.map((chapter, index) => <button key={chapter.id} className={selectedChapter?.id === chapter.id ? "active" : ""} onClick={() => void startBookChapter(chapter)}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{chapter.title}</strong>{chapter.summary && <small>{chapter.summary}</small>}{chapter.pageStart && <em>第 {chapter.pageStart}{chapter.pageEnd && chapter.pageEnd !== chapter.pageStart ? `–${chapter.pageEnd}` : ""} 頁</em>}</div></button>) : <div className="book-chapter-empty">{bookChapterMessage || "教材索引完成後，這裡會出現真實章節。"}<br /><button type="button" className="chapter-retry" onClick={() => void loadBookChapters(resource.id)}>重新整理章節</button></div>}</div>}
          </div>;
