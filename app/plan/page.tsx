...
export default function StudyPlanPage() {
  ...
  return <main className="plan-shell">
    ...
    <div className="plan-main">
      ...
      <section className="learning-hub" id="records">
      ...
      <section className="learning-hub" id="notes">
        ...
        {visibleNotes.length ? <div className="note-list">{visibleNotes.map((note) => <article key={note.id}><div><span>{note.subject}</span>{note.tags && <em>{note.tags}</em>}</div><strong>{note.title}</strong><p>{note.content}</p>{note.sourceLabel && <small>教材來源：{note.sourceLabel}</small>}</article>)}</div> : <div className="hub-empty">尚未收藏筆記。回到對話後，按下 AI 回答下方的「收藏筆記」即可加入。</div>}
