...
export default function StudyPlanPage() {
  ...
  function captureCourseFrame() {
    setCourseAiNotice("");
    const video = document.querySelector<HTMLVideoElement>(".course-player video");
    if (!video) {
      setCourseAiNotice("這堂課使用跨站內嵌播放器，瀏覽器無法直接擷取畫面；可改用裝置截圖後，到首頁 AI 對話貼上提問。");
      return;
    }
    ...
    try {
      ...
      setCourseCapture(canvas.toDataURL("image/jpeg", .82));
      setCourseAiNotice("已擷取目前畫面，可在下方直接提問。");
    } catch {
      setCourseAiNotice("影片來源未允許畫面擷取，請改用裝置截圖後，到首頁 AI 對話貼上提問。");
    }
  }
  ...
  async function askCourseAi(prompt: string, imageDataUrl = "", actionLabel = "整理答案") {
    ...
    try {
      const response = await fetch("/api/chat", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          messages: [{ role: "student", text: `${context}\n學生問題：${prompt.trim()}\n請只處理這段課程的學習問題；若畫面、摘要或教材依據不足，明確說明，不要猜測。` }],
          imageDataUrl: imageDataUrl || undefined,
        }),
      });
      const result = await response.json() as { reply?: string; error?: string };
      if (!response.ok) throw new Error(result.error || "AI 暫時無法回應");
      setCourseAiReply(result.reply || "AI 尚未產生回答，請換一種問法再試一次。");
    } catch (error) {
      setCourseAiNotice(error instanceof Error ? error.message : "AI 暫時無法回應");
    } finally {
  ...
  function submitCourseQuestion(event: FormEvent) {
    event.preventDefault();
    const question = courseAiInput.trim() || "請解釋畫面中的內容，並說明它和目前課程重點的關係。";
    void askCourseAi(question, courseCapture, courseCapture ? "解析截圖" : "回答問題");
  }
      ...
      {(activeTab === "books" || activeTab === "courses") && <section className="resource-learning-hub" aria-label={activeTab === "books" ? "智能書" : "來一課"}>
        ...
        <div className="resource-learning-layout"><aside className={`resource-list ${activeTab === "books" ? "book-resource-list" : ""}`} aria-label="可學習資源"><div className="resource-list-heading"><strong>{activeTab === "books" ? "書本清單" : "影音課程清單"}</strong><span>共 {(activeTab === "books" ? bookResources : courseResources).length} 項</span></div>{activeTab === "books" ? bookResources.map((resource) => {
          ...
          {selectedResource ? <article className="resource-study-panel"><header><div><span>{selectedResource.subject} · {selectedResource.resourceType === "book" ? "書籍" : "影音課程"}</span><h3>{selectedResource.title}</h3>{selectedResource.creator && <small>{selectedResource.creator}</small>}</div><div className="resource-panel-actions"><button className="secondary-btn" onClick={() => void addResourceTask(selectedResource)}>＋ 加入今日計畫</button><button className="primary-btn" onClick={() => void logResourceStudy(selectedResource, selectedResource.resourceType === "course" ? 45 : 60, selectedResource.resourceType === "course" ? "下次從上次字幕段落接續" : `下次從${selectedChapter?.title ? `「${selectedChapter.title}」` : "目前章節"}接續`)}>完成本次學習</button></div></header>
            ...
            {selectedResource.resourceType === "book" ? <div className="book-learning-room"><section className="book-ai-dialogue" aria-label="書籍 AI 教學"><div className="book-ai-heading"><div><span>AI 教材教學</span><strong>{selectedChapter ? selectedChapter.title : "先選一個章節"}</strong></div><small>{selectedChapter ? "依本章內容開始對話" : "從左側書本下方展開章節，AI 會直接開始教你"}</small></div>{selectedChapter ? <><div className="book-dialogue-messages">{bookMessages.map((message, index) => <div key={`${message.role}-${index}`} className={`book-dialogue-message ${message.role}`}><span>{message.role === "mentor" ? "AI 教練" : "你"}</span><p>{message.text}</p></div>)}{bookChatLoading && <div className="book-dialogue-message mentor"><span>AI 教練</span><p className="book-typing">正在整理本章內容…</p></div>}<div ref={bookDialogueEndRef} /></div><form className="book-dialogue-form" onSubmit={sendBookMessage}><textarea value={bookInput} onChange={(event) => setBookInput(event.target.value)} placeholder="回覆 AI 教練，繼續這一章…" disabled={bookChatLoading} rows={2} /><button type="submit" disabled={bookChatLoading || !bookInput.trim()}>送出</button></form></> : <div className="book-dialogue-empty"><div>AI</div><strong>選一個章節，開始學習</strong><p>這裡不顯示 PDF。AI 會依教材內容先教你抓本章重點，再用問題帶你思考。</p></div>}</section></div> : <div className="course-reader"><div className="course-player">{youtubeEmbedUrl(selectedResource.sourceUrl, selectedSegment?.startSeconds ?? selectedProgress?.positionSeconds ?? 0) ? <iframe key={`${selectedResource.id}-${selectedSegment?.id || 0}`} src={youtubeEmbedUrl(selectedResource.sourceUrl, selectedSegment?.startSeconds ?? selectedProgress?.positionSeconds ?? 0)} title={`${selectedResource.title}影音播放器`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /> : selectedResource.sourceUrl ? <CourseVideoPlayer resourceId={selectedResource.id} sourceUrl={selectedResource.sourceUrl} title={`${selectedResource.title}影音播放器`} startSeconds={selectedSegment?.startSeconds ?? selectedProgress?.positionSeconds ?? 0} seekToken={courseSeekToken} onError={setCoursePlayerError} onTimeChange={(seconds) => { const currentSecond = Math.floor(seconds); updateResourceProgress(selectedResource.id, { positionSeconds: currentSecond }); const active = [...courseSummarySegments].reverse().find((segment) => (segment.startSeconds ?? Number.POSITIVE_INFINITY) <= currentSecond); if (active && active.id !== selectedSegmentId) setSelectedSegmentId(active.id); }} /> : <div className="resource-empty">這堂課尚未設定影片網址。</div>}</div>{coursePlayerError && <div className="course-preview-error" role="alert">{coursePlayerError}</div>}<div className="course-study-meta"><div><strong>{selectedSegment ? "目前重點延伸" : "時間點重點延伸"}</strong><span>{selectedSegment?.summary || (courseSummarySegments.length ? `已整理 ${courseSummarySegments.length} 個時間點重點；點擊右側摘要即可跳到對應講解。` : "後台尚未上傳可跳轉的時間點重點。")}</span></div></div><section className="course-ai-tools" aria-label="課程重點延伸與截圖問 AI"><div className="course-ai-shortcuts"><span>接著學</span><button type="button" disabled={courseAiLoading} onClick={() => void askCourseAi("請用白話解釋目前重點，並補一個容易理解的例子。", "", "白話解釋")}>白話解釋</button><button type="button" disabled={courseAiLoading} onClick={() => void askCourseAi("請針對目前重點考我一題；先只出題，不要公布答案。", "", "出一題考我")}>考我一題</button><button type="button" disabled={courseAiLoading} onClick={() => void askCourseAi("請把目前重點整理成二試申論可用的爭點、規範、涵攝與結論架構。", "", "整理申論架構")}>申論架構</button></div>{courseAiLoading && <div className="course-ai-progress" role="status" aria-live="polite"><div className="course-ai-progress-heading"><span className="course-ai-spinner" aria-hidden="true" /><div><strong>AI 正在{courseAiAction}</strong><small>請稍候，完成後會直接顯示結果</small></div></div><ol>{[(courseCapture ? "解析圖片與畫面文字" : "讀取目前時間點與課程摘要"), "拆解內容與辨認核心考點", "核對法律概念與作答方向", "整理成清楚答案"].map((label, index) => <li key={label} className={index < courseAiStage ? "done" : index === courseAiStage ? "active" : ""}><i>{index < courseAiStage ? "✓" : index + 1}</i><span>{label}</span>{index === courseAiStage && <em>處理中…</em>}</li>)}</ol></div>}<div className="course-capture-row"><button type="button" className="course-capture-button" onClick={captureCourseFrame} disabled={courseAiLoading}>截圖問 AI</button><span>擷取目前畫面，連同時間點與摘要一起提問</span></div>{courseCapture && <div className="course-capture-preview"><img src={courseCapture} alt="目前課程畫面截圖" /><button type="button" onClick={() => setCourseCapture("")} aria-label="移除課程截圖">×</button></div>}<form className="course-ai-form" onSubmit={submitCourseQuestion}><textarea rows={2} value={courseAiInput} onChange={(event) => setCourseAiInput(event.target.value)} placeholder={courseCapture ? "這張畫面的概念是什麼？為什麼老師這樣判斷？" : "針對目前重點直接問 AI…"} disabled={courseAiLoading} /><button type="submit" disabled={courseAiLoading || (!courseAiInput.trim() && !courseCapture)}>{courseAiLoading ? "AI 思考中…" : "送出"}</button></form>{courseAiNotice && <p className="course-ai-notice">{courseAiNotice}</p>}{courseAiReply && <div className="course-ai-reply"><strong>AI 教練</strong><p>{courseAiReply}</p></div>}</section>{courseSummarySegments.length > 0 && <div className="course-segment-list">{courseSummarySegments.map((segment) => <button key={segment.id} className={selectedSegment?.id === segment.id ? "active" : ""} onClick={() => { setSelectedSegmentId(segment.id); updateResourceProgress(selectedResource.id, { segmentId: segment.id, positionSeconds: segment.startSeconds || 0 }); setCourseSeekToken((token) => token + 1); }}><span>{segment.startSeconds != null ? formatMediaTime(segment.startSeconds) : segment.sequence}</span><div><strong>{segment.title}</strong>{segment.summary && <small>{segment.summary}</small>}</div></button>)}</div>}</div>}
          </article> : <div className="resource-empty resource-empty-large">先從左側選擇一項{activeTab === "books" ? "書籍" : "影音課程"}，就在這裡開始。</div>}
        </div>
      </section>}
      {activeTab === "magazine" && <section className="magazine-library" aria-label="讀法教">
        <header className="magazine-library-head"><div><p>LAW CLASSROOM</p><h2>讀法教</h2><span>依年度與期數查找試讀文章，選取文字即可請 AI 解釋。</span></div><i>法</i></header>
