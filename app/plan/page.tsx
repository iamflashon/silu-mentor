...
type HomeFeed = { magazine: { id: number; title: string; sourceUrl: string; description?: string; isDraft?: boolean; articles?: Array<{ id: number; title: string; summary: string; reviewStatus: string; sequence: number }> } | null; listening: ListeningFeed | null; focusMusicUrl?: string };
...
export default function StudyPlanPage() {
  ...
  return <main className="plan-shell">
    ...
    <div className="plan-main">
      ...
      {activeTab === "magazine" && <section className="learning-single-column" aria-label="法教專區"><article className="column-card law-column"><div className="column-kicker">LAW CLASSROOM</div><div className="column-heading"><div><h2>法教專區</h2><span>從最新法學教室內容先抓核心爭點</span></div><i>法</i></div>{homeFeed?.magazine ? <><strong>{homeFeed.magazine.title}</strong>{homeFeed.magazine.isDraft && <p className="column-notice">目前先顯示後台匯入的試讀目錄，完整 AI 分析與發布狀態仍由後台控制。</p>}<div className="magazine-article-list">{(homeFeed.magazine.articles ?? []).map((article) => <div className="magazine-article-row" key={article.id}><div><span>{article.title}</span><small>{article.summary ? `核心爭點：${magazineIssue(article.summary)}` : (article.reviewStatus === "draft" ? "尚待後台分析／發布" : "已建立試讀分析")}</small></div></div>)}</div><a href={homeFeed.magazine.sourceUrl} target="_blank" rel="noreferrer">查看法學教室來源 →</a></> : <p className="column-empty">後台尚未匯入法學教室試讀內容。</p>}</article></section>}
