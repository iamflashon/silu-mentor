...
export async function POST(request: Request) {
  ...
  try {
    if (sourceKey.startsWith("moj-")) {
      ...
      const parsed = new XMLParser({ ignoreAttributes: false, trimValues: true }).parse(decodeArchive(bytes)); const laws = collectLawObjects(parsed); const start = body.restart ? 0 : source.importCursor; const batch = laws.slice(start, start + 20); let articleCount = 0;
