...
function sourceVariants(value: string) {
  ...
  return Array.isArray(result.sourceVariants)
    ? result.sourceVariants.filter((item): item is Record<string, unknown> =>
        Boolean(
          item &&
          typeof item === "object" &&
...
export async function GET(request: Request) {
  try {
    ...
    const listLimit = Number.isInteger(requestedLimit)
    ...
    const rows = await db
      ...
      .where(
        category && documentId
          ? and(
              eq(documents.examCategory, category),
    ...
    const fineIndexCounts = rows.length
      ? await db
          .select({
          ...
          .where(
            inArray(
              documentSearchUnits.documentId,
              rows.map((row) => row.id),
            ),
          )
          .groupBy(documentSearchUnits.documentId)
      : [];
    const fineIndexCountByDocument = new Map(
      fineIndexCounts.map((row) => [row.documentId, Number(row.total)]),
    );
    const questionCounts = await db
      ...
      .from(examQuestions)
      .groupBy(examQuestions.sourceUrl, examQuestions.subject);
    const questionStats = (row: (typeof rows)[number]) => {
      ...
      if (exact)
        return {
          ...
          draftTotal: Number(exact.draftTotal),
        };
      const sameSubject = questionCounts.filter(
        (item) => item.subject === row.subject,
      );
      return sameSubject.length === 1
        ? {
            total: Number(sameSubject[0].total),
            draftTotal: Number(sameSubject[0].draftTotal),
          }
    ...
    const [usageStats] = await db
      ...
      .from(chatMessages)
      .where(eq(chatMessages.role, "mentor"));
    const [indexSetting] = await db
      .select()
      .from(appSettings)
    ...
    return Response.json(
      {
        documents: rows.map((row) => {
          ...
          return {
            ...
            processingStage:
              row.processingStage === "queued" && row.status === "completed"
                ? "completed"
                : row.processingStage,
            ...
            indexedQuestionCount: Number(
              row.questionCount ?? counts.questionCount ?? 0,
            ),
            tags: (() => {
              try {
            ...
            assignmentCategories: row.assignmentCategories
              ? [
                  ...new Set(
                    row.assignmentCategories.split(",").filter(Boolean),
                  ),
                ]
              : [row.examCategory],
            ...
            sourceFileName:
              typeof result.sourceFileName === "string"
                ...
                : row.fileName,
            indexedFileName:
              typeof result.indexedFileName === "string"
                ? result.indexedFileName
                : row.fileName,
            sourceVariants: sourceVariants(row.processingResultJson).map(
              (item) => ({
            ...
            documentVersion:
              ...
              typeof result.documentVersion === "object"
                ? result.documentVersion
                : null,
            extractionNote:
              typeof result.extractionNote === "string"
                ? result.extractionNote
                : "",
            analysisStatus:
              typeof result.analysisStatus === "string"
        ...
        stats: {
          ...
          indexVersion: indexSetting
            ? `VS-${new Date(indexSetting.updatedAt).toISOString().slice(0, 10).replaceAll("-", "")}`
            : "待建立",
        },
      },
      {
...
export async function POST(request: Request) {
  try {
    ...
    const requestedExamCategory = String(
      form.get("examCategory") ?? "law",
    ).trim();
    const examCategory = ["law", "accounting", "medtech"].includes(
      requestedExamCategory,
    )
      ? requestedExamCategory
      : "law";
    const requestedBookTitle = normalizeDocumentTitle(
    ...
    if (!(file instanceof File) || !isSupportedDocument(file.name, file.type)) {
      return Response.json(
        { error: "請上傳 PDF、HTML、JSONL、MD、TXT、DOCX 或 ZIP 文件" },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      const [row] = await db
        ...
        .values({
          ...
          examCategory,
          bookTitle:
            requestedBookTitle || documentDisplayTitle(null, file.name),
          subject,
          documentType,
          status: "uploaded",
        })
      ...
      return Response.json(
        { document: { id: row.id, name: row.fileName, status: row.status } },
        { status: 201 },
      );
    } catch (error) {
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!ids.length)
      return Response.json({ error: "請先選擇要刪除的教材" }, { status: 400 });
    ...
    const selected = rows.filter((row): row is NonNullable<typeof row> =>
      Boolean(row),
    );
    const { env } = await import("cloudflare:workers");
    const bucket = env.BUCKET;
    ...
    for (const row of selected) {
      if (row.openaiFileId && indexSetting?.value) {
        await openAIJson(
          `/vector_stores/${indexSetting.value}/files/${row.openaiFileId}`,
          { method: "DELETE" },
        ).catch(() => undefined);
        await openAIJson(`/files/${row.openaiFileId}`, {
          method: "DELETE",
        }).catch(() => undefined);
      }
      ...
      await db
        ...
        .where(eq(examQuestions.sourceUrl, `document:${row.id}`));
    ...
    return Response.json(
      {
        error:
          error instanceof Error ? error.message.slice(0, 240) : "教材刪除失敗",
      },
      { status: 500 },
    );
  }
...
export async function PATCH(request: Request) {
  try {
    ...
    const hasHomepageSearchSetting =
      typeof body.homepageSearchEnabled === "boolean";
    if (
      !Number.isInteger(id) ||
      id < 1 ||
      (!hasBookTitle && !hasHomepageSearchSetting)
    ) {
    ...
    const [document] = await db
      ...
      .limit(1);
    if (!document)
      return Response.json({ error: "找不到這份教材" }, { status: 404 });
    if (hasBookTitle) {
    ...
    if (body.homepageSearchEnabled && document.status !== "completed") {
      return Response.json(
        {
          error: "教材仍在處理，完成全文／向量索引後才能開放首頁搜尋",
    ...
    if (
      body.homepageSearchEnabled &&
      (!document.openaiFileId || !setting?.value)
    ) {
      return Response.json(
        {
          error: "這是舊版教材索引，系統將自動補建後再開放首頁搜尋",
    ...
    if (
      ...
      const indexed = await openAIJson(
        `/vector_stores/${setting.value}/files/${document.openaiFileId}`,
      ).catch(() => null);
      if (indexed && indexed.status === "completed") {
        await db
          .update(documents)
          .set({ fullTextIndexed: true, vectorIndexed: true, indexError: null })
          .where(eq(documents.id, id));
      } else {
        return Response.json(
          {
    ...
    await db
      .update(documents)
      .set({ homepageSearchEnabled: body.homepageSearchEnabled })
      .where(eq(documents.id, id));
    return Response.json({
      id,
      homepageSearchEnabled: body.homepageSearchEnabled,
    });
  } catch (error) {
    return Response.json(
      {
