...
export async function PATCH(request: Request) {
  const auth = await requireAdmin(request);
  if ("error" in auth) return auth.error;
  const body = await request.json() as { id?: number; role?: string; status?: string; className?: string };
  const id = Number(body.id);
  if (!id) return Response.json({ error: "缺少會員編號" }, { status: 400 });
  const role = ["admin", "teacher", "student"].includes(body.role ?? "") ? body.role : undefined;
  ...
  const [updated] = await auth.db.update(members).set({ ...(role && { role }), ...(status && { status }), ...(className && { className }), updatedAt: new Date() }).where(eq(members.id, id)).returning();
