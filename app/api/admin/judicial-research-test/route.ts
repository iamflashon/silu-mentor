...
export async function POST(request: Request) {
  ...
  if ("exhausted" in auth && auth.exhausted) return Response.json({ error: "本帳號的 10 次測試額度已用完，請提出臨時調高申請。", code: "LEGAL_SEARCH_LIMIT", access: { metered: true, used: auth.used, limit: auth.limit, remaining: 0, temporaryExpiresAt: auth.temporaryExpiresAt } }, { status: 429 });
  try {
