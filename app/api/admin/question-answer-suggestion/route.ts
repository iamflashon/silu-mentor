...
export async function POST(request:Request){
 ...
 const model=await getOpenAIModel("gpt-5.6-luna"),payload=await openAIJson("/responses",{method:"POST",body:JSON.stringify({model,instructions:"你是題庫答案覆核助理。依題幹、A到D選項與既有解析獨立推論最可能答案。這只是供老師核對的建議，絕不能宣稱是官方答案。若題幹選項錯位、資料不足或無法唯一作答，mustReview 必須為 true 並說明。使用繁體中文。",input:`類科：${category}\n科目：${question.subject}\n年份：${question.year}\n題號：${question.questionNumber}\n題幹：${plain(question.stem)}\n選項：${JSON.stringify(Object.fromEntries(Object.entries(options).map(([k,v])=>[k,plain(v)])))}\n既有解析：${plain(question.explanation)||"無"}`,text:{format:{type:"json_schema",name:"answer_suggestion",strict:true,schema:{type:"object",additionalProperties:false,properties:{answer:{type:"string",enum:["A","B","C","D"]},reason:{type:"string"},confidence:{type:"string",enum:["high","medium","low"]},mustReview:{type:"boolean"}},required:["answer","reason","confidence","mustReview"]}}},max_output_tokens:900})});
 let result:{answer?:string;reason?:string;confidence?:string;mustReview?:boolean}={};try{result=JSON.parse(outputText(payload))}catch{}if(!/^[A-D]$/.test(String(result.answer||"")))return Response.json({error:"目前無法形成可靠建議，請人工核對原稿"},{status:422});
