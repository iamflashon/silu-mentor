...
export async function GET(request:Request){
 ...
 const url=new URL(request.url);const id=Number(url.searchParams.get("id"));const variant=url.searchParams.get("variant")||"";const db=await getDb();const [doc]=await db.select().from(documents).where(and(eq(documents.id,id),eq(documents.examCategory,"medtech"))).limit(1);if(!doc)return new Response("Not found",{status:404});
