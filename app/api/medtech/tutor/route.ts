...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as {
      ...
      level?: string;
      messages?: Turn[];
      mode?: "hint" | "compare" | "answer" | "followup";
    ...
    const messages = (body.messages ?? [])
      .filter(
        (item) =>
          item &&
          ["student", "mentor"].includes(item.role) &&
    ...
    if (!question)
      return Response.json(
        { error: "找不到這道醫檢師題目，請重新抽題。" },
        { status: 404 },
      );
    const level = ["入門", "進階", "考前衝刺"].includes(String(body.level))
      ? String(body.level)
      : "入門";
    const selectedAnswer = String(
      body.selectedAnswer ?? latest.match(/^我選\s*([A-D])/iu)?.[1] ?? "",
    ).toUpperCase();
    ...
    if (mode === "compare" && !selectedAnswer)
      return Response.json(
        { error: "請先選擇答案，再比較選項。" },
        { status: 400 },
      );
    ...
    if (cacheMode) {
      const [cached] = await db
        ...
        .where(eq(medtechAiExplanationCache.cacheKey, cacheKey))
        .limit(1);
      if (cached) {
        await db
          .update(medtechAiExplanationCache)
          .set({ lastUsedAt: new Date() })
          .where(eq(medtechAiExplanationCache.id, cached.id));
        return Response.json({
          reply: cached.reply,
          source:
            mode === "hint"
              ...
              : mode === "compare"
                ? "選項比較（已預先整理）"
                : "答題結果（已預先整理）",
          creditsRemaining: usageState.aiCredits,
          usage: {
            model: "stored",
            inputTokens: 0,
            outputTokens: 0,
    ...
    return Response.json(
      {
        error:
          "即時 AI 追問目前暫停開放。請使用已整理的解題提示、選項比較、完整解析與老師語音。",
        code: "LIVE_AI_DISABLED",
      },
      { status: 503 },
    );
    const options = JSON.parse(question.optionsJson || "{}") as Record<
      string,
      string
    >;
    const activeAnswer =
    ...
    const conversation = messages
      .map(
        (item) =>
          `${item.role === "student" ? "學生" : "醫檢 AI 助教"}：${item.text.slice(0, 1800)}`,
      )
      .join("\n\n");
    ...
    const estimatedCostUsdMicros = estimateCostUsdMicros(model, {
      inputTokens,
      outputTokens,
      cachedTokens,
    });
    if (cacheMode) {
      await db
        .insert(medtechAiExplanationCache)
        .values({
          cacheKey,
    ...
    if (!updatedUsage)
      return Response.json(
        {
          error: "點數已用完；AI 追問每題扣 1 點，請先購買點數。",
          code: "POINTS_EXHAUSTED",
          upgradeUrl: "/medtech/upgrade?reason=points",
        },
        { status: 402 },
      );
    await db.insert(usageLogs).values({
    ...
    return Response.json({
      ...
      source: question.explanation?.trim()
        ? "教材答案與原稿解析"
        : "教材答案＋AI 補充",
      creditsRemaining: updatedUsage.aiCredits,
      usage: {
        model: "Luna",
        inputTokens,
        outputTokens,
    ...
    return Response.json(
      { error: error instanceof Error ? error.message : "醫檢 AI 回答失敗" },
      { status: 500 },
    );
  }
}
