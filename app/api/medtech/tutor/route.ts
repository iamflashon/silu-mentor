...
export async function POST(request: Request) {
  try {
    ...
    const payload = await openAIJson("/responses", { method: "POST", body: JSON.stringify({
      model,
      instructions: `你是臺灣醫事檢驗師國考的 AI 學習助教，學生程度為「${level}」。你只能討論醫事檢驗、臨床病毒學及所附題庫資料，絕不可引用或混入司律、法律教材。以繁體中文教學。先回應學生真正的疑問，再用必要的基礎概念說明；比較選項時逐項指出判斷關鍵。教材答案是核對基準。若原稿沒有文字解析，必須明示「以下為 AI 補充說明」，不可假稱教材原文。若學生尚未要求答案，可先用一個提示或追問引導；若明確要求答案、解析或比較選項，就直接完整回答。不要捏造頁碼、文獻或教材內容。結尾提出一個簡短的理解確認問題。`,
      input: `${evidence}\n\n【本次對話】\n${conversation}`,
