...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      await db.insert(usageLogs).values({ model, source: "醫檢｜框選白話解析", inputTokens, cachedTokens, outputTokens, fileSearchCalls: 0, estimatedCostUsdMicros });
    } catch { /* 成本紀錄失敗不阻擋學生取得解析 */ }
    return Response.json({ ...parsed, usage: { model, inputTokens, cachedTokens, outputTokens, durationMs, estimatedCostUsd: estimatedCostUsdMicros / 1_000_000 } });
  } catch (error) {
