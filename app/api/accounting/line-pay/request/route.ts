...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => ({})) as { plan?: string; chapterNumber?: number };
  ...
  try {
    ...
    const chapterNumber = Math.max(1, Math.min(18, Math.floor(Number(body.chapterNumber || 1)))); const chapterPlan = body.plan === "chapter"; const amount = chapterPlan ? 39 : product.effectivePrice; const accessDays = chapterPlan ? 30 : product.accessDays; const packageName = chapterPlan ? `accounting:chapter:${chapterNumber}` : `accounting:${ACCOUNTING_FIRST_PRODUCT_KEY}`;
    const orderId = `AC${Date.now()}${crypto.randomUUID().replaceAll("-", "").slice(0, 10)}`;
    const origin = new URL(request.url).origin;
    await auth.db.insert(medtechPaymentOrders).values({ userKey: auth.userKey, orderId, environment: config.environment, packageName, packNumber: chapterPlan ? chapterNumber : 1, amount, currency: "TWD", status: "pending" });
    const result = await linePayPost("/v3/payments/request", { amount, currency: "TWD", orderId, packages: [{ id: packageName, amount, products: [{ id: packageName, name: chapterPlan ? `${product.title}・第 ${chapterNumber} 章 ${accessDays} 天` : `${product.title}・整本 ${accessDays} 天`, quantity: 1, price: amount }] }], redirectUrls: { confirmUrl: `${origin}/api/accounting/line-pay/confirm?orderId=${encodeURIComponent(orderId)}`, cancelUrl: `${origin}/api/accounting/line-pay/cancel?orderId=${encodeURIComponent(orderId)}` } });
