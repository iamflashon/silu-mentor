...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (paid) { const product = await getAccountingProductSettings(auth.db), now = new Date(), chapterPlan = order.packageName.startsWith("accounting:chapter:"), entitlementKey = chapterPlan ? `${ACCOUNTING_FIRST_PRODUCT_KEY}:chapter:${order.packNumber}` : ACCOUNTING_FIRST_PRODUCT_KEY, accessDays = chapterPlan ? 30 : product.accessDays; const [current] = await auth.db.select().from(accountingMemberEntitlements).where(and(eq(accountingMemberEntitlements.memberId, auth.member.id), eq(accountingMemberEntitlements.productKey, entitlementKey))).limit(1); const base = current?.status === "active" && current.expiresAt > now ? current.expiresAt : now; const expiresAt = new Date(base.getTime() + accessDays * 86400000); await auth.db.insert(accountingMemberEntitlements).values({ memberId: auth.member.id, productKey: entitlementKey, status: "active", source: "line_pay", startsAt: now, expiresAt, note: `LINE Pay ${order.amount} 元開通${chapterPlan ? `第 ${order.packNumber} 章` : "整本"} ${accessDays} 天`, updatedBy: "line_pay" }).onConflictDoUpdate({ target: [accountingMemberEntitlements.memberId, accountingMemberEntitlements.productKey], set: { status: "active", source: "line_pay", expiresAt, note: `LINE Pay ${order.amount} 元開通${chapterPlan ? `第 ${order.packNumber} 章` : "整本"} ${accessDays} 天`, updatedBy: "line_pay", updatedAt: now } }); }
