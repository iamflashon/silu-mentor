...
export async function POST(request:Request){
  ...
  const extension=file.type.split("/")[1].replace("jpeg","jpg");const key="accounting/question-assets/"+crypto.randomUUID()+"."+extension;
  const {env}=await import("cloudflare:workers");await env.BUCKET.put(key,file.stream(),{httpMetadata:{contentType:file.type}});
  return Response.json({url:"/api/accounting/admin/question-assets?key="+encodeURIComponent(key)});
}
export async function GET(request:Request){
  const key=new URL(request.url).searchParams.get("key")||"";if(!/^accounting\/question-assets\/[a-f0-9-]{36}\.(?:png|jpg|webp|gif)$/.test(key))return new Response("Not found",{status:404});
