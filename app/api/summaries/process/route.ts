...
export async function POST(request: Request) {
  let documentId = 0;
  try {
    const body = await request.json() as { id?: number; model?: "luna" | "sol" };
    ...
    const model = body.model === "sol" ? await getEssayOpenAIModel("gpt-5.6-sol") : await getOpenAIModel("gpt-5.6-luna");
    await db.update(documents).set({ status: "analyzing", processingStage: "analyzing", processingMessage: `AI 正在整理摘要（${body.model === "sol" ? "Sol" : "Luna"}）`, indexError: null }).where(eq(documents.id, documentId));
    ...
    const payload = await fetch("https://api.openai.com/v1/responses", {
      method: "POST",
      headers: { ...(await openAIHeaders(true)) },
      body: JSON.stringify({
        model,
        ...
        input: [{ role: "user", content: [
          ...content,
          { type: "input_text", text: `請整理檔案「${document.fileName}」，科目為「${document.subject}」。輸出：核心摘要、考試重點、重要爭點、常見錯誤、來源頁碼或原文位置、標籤，以及 3 至 8 張問答複習卡。若不是法律教材，也請依實際內容整理，不要假設它是法律。` },
        ] }],
