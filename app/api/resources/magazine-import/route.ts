...
export async function POST(request: Request) {
  ...
  for (let index = 0; index < articles.length; index++) { const article = articles[index]; const row = current.find((item) => item.title === article.title.slice(0, 100)); try { const analyzed = await analyzeTrialPdf(article); const values = { segmentType: "article_trial", lessonLabel: title, title: article.title.slice(0, 100), sourceUrl: article.url, text: analyzed.text, summary: analyzed.analysis, importance: 5, recommended: true, reviewStatus: "ai_reviewed", sequence: index + 1 }; if (row) await db.update(resourceSegments).set(values).where(eq(resourceSegments.id, row.id)); else await db.insert(resourceSegments).values({ resourceId: resource.id, ...values }); await db.insert(usageLogs).values({ model: analyzed.model, source: "月旦試讀PDF", inputTokens: analyzed.usage?.input_tokens ?? 0, cachedTokens: analyzed.usage?.input_tokens_details?.cached_tokens ?? 0, outputTokens: analyzed.usage?.output_tokens ?? 0, fileSearchCalls: 0, estimatedCostUsdMicros: 0 }); indexed++; } catch (error) { const failureMessage = error instanceof Error ? error.message : "處理失敗"; failures.push(`${article.title}：${failureMessage}`); const values = { segmentType: "article_link", lessonLabel: title, title: article.title.slice(0, 100), sourceUrl: article.url, text: JSON.stringify({ ...article, status: "failed", error: failureMessage }), summary: "", importance: 0, recommended: false, reviewStatus: "failed", sequence: index + 1 }; if (row) await db.update(resourceSegments).set(values).where(eq(resourceSegments.id, row.id)); else await db.insert(resourceSegments).values({ resourceId: resource.id, ...values }); } }
