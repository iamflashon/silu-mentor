...
async function analyzeTrialPdf(article: { title: string; url: string }) {
  ...
  const payload = await openAIJson("/responses", { method: "POST", body: JSON.stringify({ model: process.env.OPENAI_MODEL || "gpt-5.6-luna", instructions: "你是台灣司律考試期刊編輯。只能整理公開試讀 PDF 可見內容，不得補造全文。最重要的輸出是 issue。第一優先尋找版面中明示『爭點』的標題，忠實擷取該標題下方的完整可見段落；不要把文章標題、一般摘要、案例結論或寬泛主題詞當作爭點。只有在試讀頁完全沒有『爭點』標題時，才可依文章開頭整理一個具體法律疑問，並在 issueSource 回傳 inferred。issue 應保留當事人行為、程序階段、法條與法律判斷的分岔。text 保留作者、正文、案例、法條、爭點與試讀範圍的完整可見文字，以供 AI 檢索。不得使用 Markdown 星號。", input: [{ role: "user", content: [{ type: "input_file", filename: article.url.split("/").pop() || "trial.pdf", file_data: `data:application/pdf;base64,${Buffer.from(bytes).toString("base64")}` }, { type: "input_text", text: `文章：${article.title}\n請先辨識版面中的「爭點」標題，擷取其下方完整段落，再整理其他可見內容。` }] }], text: { format: { type: "json_schema", name: "magazine_trial", strict: true, schema: { type: "object", additionalProperties: false, properties: { author: { type: "string" }, text: { type: "string" }, issue: { type: "string" }, issueSource: { type: "string", enum: ["explicit", "inferred", "missing"] }, legalTopics: { type: "array", items: { type: "string" } } }, required: ["author", "text", "issue", "issueSource", "legalTopics"] } } } }) });
  const parsed = JSON.parse(outputText(payload)) as { author: string; text: string; issue: string; issueSource: "explicit" | "inferred" | "missing"; legalTopics: string[] }; const usage = payload.usage as { input_tokens?: number; output_tokens?: number; input_tokens_details?: { cached_tokens?: number } } | undefined;
  const issue = parsed.issue.trim();
  if (!issue || parsed.issueSource === "missing") throw new Error("已讀取試讀 PDF，但沒有擷取到明確爭點；請改由後台人工補上");
  return { ...parsed, issue, text: `主要爭點：${issue}\n\n${parsed.text.trim()}`, model: String(payload.model ?? process.env.OPENAI_MODEL ?? "gpt-5.6-luna"), usage };
}
