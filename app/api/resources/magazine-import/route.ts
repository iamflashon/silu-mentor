...
async function analyzeTrialPdf(article: { title: string; url: string }) {
  ...
  const payload = await openAIJson("/responses", { method: "POST", body: JSON.stringify({ model: process.env.OPENAI_MODEL || "gpt-5.6-luna", instructions: "你是台灣司律考試期刊編輯。只能整理公開試讀PDF可見內容，不得補造全文。最重要的任務是辨識文章真正的『核心法律爭點』，不要只回傳寬泛的主題詞。擷取作者、正文、問題或案例、爭點、法條與試讀範圍；text保留可供AI檢索的完整可見文字；issuePoints列出1至4個可直接讓考生理解本篇在考什麼的爭點；summary以繁中80到160字整理考試價值，第一句必須先說明核心爭點。不得使用Markdown星號。", input: [{ role: "user", content: [{ type: "input_file", filename: article.url.split("/").pop() || "trial.pdf", file_data: `data:application/pdf;base64,${Buffer.from(bytes).toString("base64")}` }, { type: "input_text", text: `文章：${article.title}\n請整理這份公開試讀內容，優先找出文中標示的「爭點」段落與其法律問題。` }] }], text: { format: { type: "json_schema", name: "magazine_trial", strict: true, schema: { type: "object", additionalProperties: false, properties: { author: { type: "string" }, text: { type: "string" }, summary: { type: "string" }, issuePoints: { type: "array", items: { type: "string" } } }, required: ["author", "text", "summary", "issuePoints"] } } } }) });
  const parsed = JSON.parse(outputText(payload)) as { author: string; text: string; summary: string; issuePoints: string[] }; const usage = payload.usage as { input_tokens?: number; output_tokens?: number; input_tokens_details?: { cached_tokens?: number } } | undefined;
  const issuePoints = Array.isArray(parsed.issuePoints) ? parsed.issuePoints.map((item) => item.trim()).filter(Boolean).slice(0, 4) : [];
  const issuePrefix = issuePoints.length ? `爭點：${issuePoints.join("；")}｜` : "爭點：";
  return { ...parsed, summary: `${issuePrefix}${parsed.summary.trim()}`.slice(0, 600), model: String(payload.model ?? process.env.OPENAI_MODEL ?? "gpt-5.6-luna"), usage };
}
