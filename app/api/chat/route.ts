    ...
    const bookEvidenceInstruction = context.type === "book"
      ? bookEvidence?.status === "verified"
        ? bookEvidence.basis === "teacher_solution"
          ? `\n\n【本題老師解析／擬答（唯一的老師見解來源）】\n書名：${bookEvidence.resourceTitle}\n題型：${bookEvidence.segmentTitle}\n位置：${bookEvidence.pageStart ? `第 ${bookEvidence.pageStart}${bookEvidence.pageEnd && bookEvidence.pageEnd !== bookEvidence.pageStart ? `–${bookEvidence.pageEnd}` : ""} 頁` : "待核對"}\n老師解析／擬答原文：${bookEvidence.excerpt}\n\n這是解題書教學，不是一般知識問答。你的任務是忠實拆解上方同一題的老師解析，不是重新作答。請嚴格遵守以下來源邊界：\n1. 「老師解析的核心爭點」只能列出上方原文明示或可直接對應的爭點，不得依一般解題習慣自行增列。\n2. 每個核心爭點後都要附一段「老師原文依據」，使用原文中的關鍵句或忠實摘述，讓學生能逐項核對。\n3. 「白話拆解」只能解釋該原文的規範、涵攝、結論與得分重點，不得把 AI 新增內容寫成老師主張。\n4. 上方原文沒有處理、但你認為值得提醒的內容，只能放在最後獨立的「AI 延伸檢查」；沒有必要就完全省略。\n5. 不得因題目出現某人物，就自行替該人物新增老師未討論的罪名。例如老師只處理被利用人與間接正犯時，不得把被利用人的過失犯罪列為老師核心爭點。\n6. 若學生問「老師怎麼看」或要求整理本題，依序輸出「老師解析的核心爭點」「老師原文依據」「白話拆解」；如有額外內容才加「AI 延伸檢查」。\n7. 不得使用「依本題老師解析」標章包裝任何無法由原文直接核對的主張。`
