...
export async function POST(request: Request) {
  try {
    ...
    const officialAuthorityInstruction = authorityReviewRequired
      ? `\n\n【程序與裁判實務強制覆核】
本題涉及法院程序要件、准駁或實務見解。回答前必須分開檢查：一、法條是否容許提出聲請；二、法院是否可能因法定要件不具備而直接駁回；三、即使核發，債務人異議後的程序效果。不得把「可以遞狀」「請求人自行填寫金額」直接等同「符合要件、法院會核發」。必須區分法條明文、裁判實務與個案推論；不同層次不得混寫。若下列同步裁判沒有直接處理同一命題，必須明說目前資料不足或實務可能有不同處理，不得用模型記憶補成一致見解。${officialCaseEvidence.length ? `\n\n【平台同步的司法院裁判資料】\n${officialCaseEvidence.map((item, index) => `【裁判 ${index + 1}｜${item.label}】\n${item.excerpt}`).join("\n\n")}` : "\n本次平台同步資料尚未命中直接相關裁判；請採保留式結論，提醒需再查核，不得下絕對結論。"}`
      : "";
