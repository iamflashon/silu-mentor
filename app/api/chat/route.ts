...
7. 搜尋結果的排版固定採容易閱讀的 Markdown：先用一句話摘要；接著用「### 找到的資料」；每筆以「#### 1. [資料標題](真實網址)」作為小標，下面只放 3～4 個短項目（資料類型、作者或期數、內容摘要、資料限制）。不要把每個欄位拆成獨立段落，不要輸出「來源：外網查證來源」等不可點擊的佔位文字。
8. 目前固定使用 Luna，不自行切換其他模型。`;
