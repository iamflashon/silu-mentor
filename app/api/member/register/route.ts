...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (existing) return Response.json({ error: "這個 Email 已經註冊，請直接登入。" }, { status: 409 });

    stage = "password-hash";
    const passwordHash = await hashMemberPassword(password);
    stage = "member-insert";
    const [member] = await db.insert(members).values({ email, passwordHash, displayName, role: "student", status: "active", className: "未分班", lastSeenAt: new Date() }).returning({ id: members.id, email: members.email });

    stage = "session-cookie";
    const cookie = await createMemberSessionCookie({ memberId: member.id, email: member.email });
    if (!cookie) return Response.json({ error: "註冊服務尚未完成設定，請聯絡管理員。" }, { status: 503 });
    return Response.json({ ok: true, returnTo: safeReturnTo(body.returnTo) }, { status: 201, headers: { "cache-control": "no-store", "set-cookie": cookie } });
  } catch (error) {
    console.error({
      event: "member-registration-failed",
      stage,
      error: error instanceof Error ? { name: error.name, message: error.message } : String(error),
    });
    return Response.json({ error: "註冊服務暫時無法使用，請稍後再試。" }, { status: 503 });
  }
}
