...
type CoachMessage = { id: string; role: "student" | "coach" | "scholar"; text: string; source?: string; replyTo?: { id: string; excerpt: string } };
...
export default function PengliCoach() {
  const [messages, setMessages] = useState<CoachMessage[]>([{ id: "welcome", role: "coach", text: "我是彭狸 AI 教練。這裡只依彭狸老師《行政法考點（考前衝刺）演習書》的學習脈絡陪你練習；我會先幫你找爭點與破題方向，不會一開始就把整份擬答貼給你。", source: "專區使用說明" }]);
  ...
  const [verification, setVerification] = useState<{ ticketId: number; text: string; sources: { label: string; url?: string }[]; escalated?: boolean } | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(saved) && saved.length) setMessages(saved.slice(-40).map((message) => ({ ...message, id: message.id || crypto.randomUUID() })));
  ...
  useEffect(() => { void fetch("/api/teachers/pengli/questions", { cache: "no-store" }).then(async response => response.ok ? response.json() : null).then(data => setUnreadCount(Number(data?.unreadCount || 0))).catch(() => undefined); }, []);
  ...
  async function requestCoach(next: CoachMessage[]) {
    ...
    setMessages((current) => [...current, { id: crypto.randomUUID(), role: "coach", text: data.reply!, source: data.source }]);
  ...
  async function ask(text: string) {
    ...
    const quoted = replyTarget ? `針對這段回覆：「${replyTarget.text.slice(0, 240)}」\n\n${question}` : question;
    const next = [...messages, { id: crypto.randomUUID(), role: "student" as const, text: quoted, replyTo: replyTarget ? { id: replyTarget.id, excerpt: replyTarget.text.slice(0, 120) } : undefined }];
    setMessages(next);
    setInput("");
    setReplyTarget(null);
    setThinking(true);
  ...
  async function askScholarToAnswer() {
    ...
    try {
      ...
      const next = [...messages, { id: crypto.randomUUID(), role: "scholar" as const, text: data.scholarDraft, source: "學霸代答（學生角色）" }];
      setMessages(next);
  ...
  async function verifyDoubt() {
    ...
    setDoubtLoading(true); setError(""); setVerification(null);
    try {
      const response = await fetch("/api/teachers/pengli/coach", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode: "verify-doubt", messageKey: doubtTarget.id, aiReply: doubtTarget.text, studentQuestion: doubtText, topic: new URLSearchParams(window.location.search).get("topic") || "行政法", conversationKey: storageKey, requestKey: crypto.randomUUID() }) });
      const data = await response.json() as { verification?: string; ticketId?: number; sources?: { label: string; url?: string }[]; access?: Access; error?: string };
      if (!response.ok || !data.verification || !data.ticketId) throw new Error(data.error || "目前無法完成查證。");
      setVerification({ ticketId: data.ticketId, text: data.verification, sources: data.sources || [] });
      if (data.access) setAccess(data.access);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "目前無法完成查證。"); }
    finally { setDoubtLoading(false); }
  }

  async function escalateDoubt() {
    if (!verification) return;
    const response = await fetch("/api/teachers/pengli/questions", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: verification.ticketId, action: "escalate" }) });
  ...
  return <section className={`pengli-coach-shell${chatMaximized ? " chat-maximized" : ""}`}>
    <aside className="pengli-coach-sidebar">
      ...
      <button type="button" onClick={() => { setMessages([{ id: crypto.randomUUID(), role: "coach", text: "新的練習開始了。請貼上行政法題目，或告訴我你正在讀哪一個考點。", source: "彭狸 AI 教練" }]); setUsage(null); setError(""); }}>＋ 另開練習</button>
    </aside>
    ...
    <div className="pengli-coach-main">
      ...
      <div className="pengli-coach-thread" aria-live="polite">
        ...
        {messages.map((message) => <article data-selection-scope="pengli" data-selection-source={message.source || ""} className={message.role === "coach" ? "coach" : "student"} key={message.id}>
          <div className="pengli-coach-avatar">{message.role === "coach" ? "狸" : "我"}</div>
          <div>{message.replyTo && <blockquote>回覆：{message.replyTo.excerpt}…</blockquote>}<small>{message.role === "coach" ? "彭狸 AI 教練" : message.role === "scholar" ? "我的回答（學霸幫我答）" : "我的問題"}</small><p>{message.text}</p>{message.source && <span>（根據《{message.source}）</span>}{message.role === "coach" && <nav className="pengli-message-actions"><button type="button" onClick={() => { setReplyTarget(message); setDoubtTarget(null); }}>↩ 針對這段追問</button><button type="button" onClick={() => { setDoubtTarget(message); setDoubtText(""); setVerification(null); }}>？ 我有疑問</button></nav>}</div>
        </article>)}
      ...
      {doubtTarget && <section className="pengli-doubt-panel"><button type="button" className="close" onClick={() => { setDoubtTarget(null); setVerification(null); }}>×</button><b>針對這則 AI 回覆提出疑問</b><blockquote>{doubtTarget.text.slice(0, 300)}{doubtTarget.text.length > 300 ? "…" : ""}</blockquote>{!verification ? <><textarea value={doubtText} onChange={(event) => setDoubtText(event.target.value)} placeholder="寫下你認為不正確、不完整，或想確認的地方……" /><button type="button" onClick={() => void verifyDoubt()} disabled={!doubtText.trim() || doubtLoading}>{doubtLoading ? "正在查證官方法規與裁判…" : "使用本組查證機會"}</button></> : <div className="pengli-verification"><h3>AI 外部查證結果</h3><p>{verification.text}</p>{verification.sources.length > 0 && <ul>{verification.sources.map((source) => <li key={source.label}>{source.url ? <a href={source.url} target="_blank" rel="noreferrer">{source.label}</a> : source.label}</li>)}</ul>}{verification.escalated ? <strong>已送交彭狸老師，回覆後會在「我的筆記」通知你。</strong> : <button type="button" onClick={() => void escalateDoubt()}>仍有疑問，轉請彭狸老師</button>}</div>}</section>}
      ...
      <form className="pengli-coach-composer" onSubmit={submit}>
        {replyTarget && <div className="pengli-reply-target"><span>正在回覆：{replyTarget.text.slice(0, 100)}…</span><button type="button" onClick={() => setReplyTarget(null)}>×</button></div>}
      ...
      <footer><span>AI 分身不等同真人老師；每完成 5 輪陪練扣 1 次。</span><a className="pengli-notes-link" href="/teachers/pengli/notes">✉ 我的筆記{unreadCount > 0 ? `（${unreadCount} 封新回覆）` : ""}</a><a className="pengli-mobile-access" href="/teachers/pengli/ai-access">購買／兌換碼</a><small>{access?.coachRoundsUsed ?? 0}／{access?.coachRoundsTarget ?? 5} 輪・剩餘 {access?.remaining ?? "—"} 次</small></footer>
    </div>
  </section>;
}
