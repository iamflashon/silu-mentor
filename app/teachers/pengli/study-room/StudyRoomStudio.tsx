...
export default function StudyRoomStudio({ adminMode = false }: { adminMode?: boolean }) {
  ...
  const makePrompt = (targetTopic: string) => active.prompt.replaceAll("{topic}", targetTopic).replaceAll("{concept}", concept.trim() || `「${targetTopic}」中最重要的概念`).replaceAll("{answer}", answer.trim() || "（尚未填寫）").replaceAll("{mc}", String(mc)).replaceAll("{short}", String(short)).replaceAll("{essay}", String(essay)).replaceAll("{minutes}", String(minutes));
  const prompt = useMemo(() => makePrompt(topic), [active, topic, concept, answer, mc, short, essay, minutes]);
  const topicBatchEnabled = adminMode && topicBatchTools.includes(active.id);
  const canGenerateAlongsideExisting = active.id === "quiz" || (topicBatchEnabled && batchScope !== "current" && !(active.id === "explain" && concept.trim()));
  async function run() {
    ...
    try {
      ...
      const total = adminMode && active.id === "quiz" ? batchCount : targetTopics.length;
      ...
      setResultNote(total > 1 ? `已依序完成 ${generated.length} ${active.id === "quiz" ? "題" : "個教材主題"}，每份均已分開存入成果庫並列為待審核。` : latestData.cached ? "已從共用成果庫載入，本次不扣使用次數。" : latestData.saved ? (adminMode ? "已存入成果庫並列為待審核；請在上方確認後發布到前台。" : "已自動保存；內容會先由後台審核，發布後才供其他同學共用。") : "");
      if (adminMode) {
  ...
  return <div className="studio-shell">
    ...
    <section className="studio-workspace"><header><div><span>學習範本 {active.no}</span><h2>{active.title}</h2><p>{active.description}</p></div>{!adminMode && <div className="studio-header-actions"><button className="published-toggle" onClick={() => void loadPublished(true)}>已發布教材{published.length ? ` ${published.length}` : ""}</button><button className="history-toggle" onClick={() => { setShowPublished(false); void loadHistory(); }}>{showHistory ? "返回學習工具" : "我的學習紀錄"}</button></div>}</header>{showPublished ? <section className="study-history published-library"><h3>老師已發布的學習內容</h3><p>直接開啟閱讀，不需再次產生，也不扣使用次數。</p>{publishedLoading ? <p>正在讀取…</p> : published.length ? published.map((item) => <button key={item.id} onClick={() => { const found = tools.find((tool) => tool.id === item.tool); if (found) setActive(found); setResult(item.content); setSource(item.sourceLabel); setResultNote("這是老師已審核發布的共用內容，本次不扣使用次數。"); setShowPublished(false); }}><b>{tools.find((tool) => tool.id === item.tool)?.title || item.tool}</b><span>{item.topic}</span><small>直接閱讀</small></button>) : <p>目前尚無已發布內容。</p>}</section> : showHistory ? <section className="study-history"><h3>最近自動保存的內容</h3>{historyLoading ? <p>正在讀取…</p> : history.length ? history.map((item) => <button key={item.id} onClick={() => { setResult(item.outputText); setSource(item.sourceLabel); setResultNote(item.cacheHit ? "當時由共用成果庫載入，未扣使用次數。" : "當時已自動保存。" ); setShowHistory(false); }}><b>{tools.find((tool) => tool.id === item.tool)?.title || item.tool}</b><span>{item.topic}</span><small>{new Date(item.createdAt).toLocaleString("zh-TW")}</small></button>) : <p>目前還沒有學習紀錄。</p>}</section> : <div className="studio-form">
      ...
      <details className="prompt-template"><summary>查看本次學習範本</summary><pre>{prompt}</pre></details>{adminMode && currentArtifact && <div className={`admin-artifact-state ${currentArtifact.reviewStatus === "published" ? "published" : "pending"}`}><b>{currentArtifact.reviewStatus === "published" ? "此主題已有已發布內容" : "此主題已有內容，尚待審核"}</b><span>{active.id === "quiz" ? "可繼續批次產生不重複題目；新題會分開存入待審核清單。" : canGenerateAlongsideExisting ? "目前主題會跳過，系統將依序處理其他尚未完成的教材主題。" : currentArtifact.reviewStatus === "published" ? "學生前台已可直接閱讀，不需要再次產生。" : "請先查看內容，再到上方成果發布管理進行發布。"}</span></div>}{adminMode && currentArtifact && !canGenerateAlongsideExisting ? <button className="studio-run existing" onClick={() => { setResult(currentArtifact.content); setSource(currentArtifact.sourceLabel); setResultNote(currentArtifact.reviewStatus === "published" ? "此內容已發布到學生前台。" : "此內容尚待審核，尚未出現在學生前台。"); }}>{currentArtifact.reviewStatus === "published" ? "查看已發布內容" : "查看待審內容"}</button> : <button className="studio-run" onClick={run} disabled={loading}>{loading ? active.id === "quiz" && adminMode ? "正在依序產生題目…" : topicBatchEnabled && batchScope !== "current" ? "正在依序處理教材主題…" : "正在依教材準備…" : active.id === "quiz" ? adminMode ? `批次產生 ${batchCount} 題` : "開始第一題" : topicBatchEnabled && batchScope !== "current" && !(active.id === "explain" && concept.trim()) ? batchScope === "all" ? "批次產生全部未完成主題" : "批次產生 5 個未完成主題" : active.id === "teach" ? "請檢查我的說明" : `產生${active.title}`}</button>}{error && <p className="studio-error">{error}</p>}
