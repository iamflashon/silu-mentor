...
type Artifact = { id: number; tool: string; topic: string; content: string; sourceLabel: string; reviewStatus: string; reuseCount: number; updatedAt: string };
const labels: Record<string, string> = { guide: "完整讀書指南", quiz: "反過來考我", priority: "考前重點排序", explain: "概念拆解", gaps: "教材銜接缺口", mock: "完整模擬考", audio: "通勤語音摘要", teach: "換你教一次" };
...
export default function ArtifactManager() {
  ...
  return <section className="artifact-manager">
    ...
    <div className="artifact-list">{rows.length ? rows.map((row) => <article key={row.id} className={selected.includes(row.id) ? "selected" : ""}>
      ...
      <div className="artifact-body"><small>{labels[row.tool] || row.tool}</small><h3>{row.topic}</h3>{editingId === row.id ? <div className="artifact-editor"><label>內容<textarea rows={14} value={draftContent} onChange={(event) => setDraftContent(event.target.value)} /></label><label>教材來源註記<input value={draftSourceLabel} onChange={(event) => setDraftSourceLabel(event.target.value)} /></label><div><button onClick={() => void saveEdit()} disabled={!draftContent.trim() || busy}>儲存修改</button><button className="secondary" onClick={() => setEditingId(null)} disabled={busy}>取消</button></div></div> : <><p>{row.content.slice(0, 180)}{row.content.length > 180 ? "…" : ""}</p><em>共用 {row.reuseCount} 次</em></>}</div>
