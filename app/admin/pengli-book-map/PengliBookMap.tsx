    ...
    <div className="pengli-map-grid">
      <aside className="pengli-map-sections">
        ...
        {sections.map((section, index) => <button type="button" key={section.sectionKey} className={`${index === active ? "active" : ""} ${section.sectionType === "front_matter" ? "front" : ""}`} onClick={() => { setActive(index); setPage(section.pdfStartPage || Math.max(23, page)); }}>
          <span>{index === 0 ? "前置" : String(index).padStart(2, "0")}</span><div><b>{section.title}</b><small>{section.pdfStartPage > 0 && section.pdfEndPage >= section.pdfStartPage ? `PDF ${section.pdfStartPage}–${section.pdfEndPage} 頁` : section.pdfStartPage > 0 ? `起點 PDF ${section.pdfStartPage} 頁，尚未設定終點` : "尚未對照"}</small></div><i>{section.verified ? "已核對" : "待核對"}</i>
        </button>)}
      </aside>
      <section className="pengli-map-workspace">
        <header><div><b>{selected?.title}</b><small>目前檢視 PDF 第 {page} 頁</small></div><div className="pengli-page-nav"><button onClick={() => setPage((value) => Math.max(1, value - 1))}>上一頁</button><input aria-label="PDF 頁碼" type="number" min="1" max={book.totalPages || undefined} value={page} onChange={(event) => setPage(Math.max(1, Number(event.target.value) || 1))}/><button onClick={() => setPage((value) => Math.min(book.totalPages || 9999, value + 1))}>下一頁</button></div></header>
        {book.hasPdf ? <iframe key={page} title="私有 PDF 原稿" src={`/api/admin/pengli-book-map/source?id=${book.id}#page=${page}&zoom=page-width`}/> : <div className="pengli-pdf-empty"><b>目前只有逐頁索引，尚未保存私有 PDF 原稿</b><p>上傳後只供管理後台核對，學生端不會取得 PDF。</p><label>{uploading ? "上傳中…" : "上傳受保護的 PDF 原稿"}<input type="file" accept="application/pdf,.pdf" disabled={uploading} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); }}/></label></div>}
        <div className="pengli-map-controls"><div><label>起始頁<input type="number" min="0" value={selected?.pdfStartPage || 0} onChange={(event) => update(active, "pdfStartPage", Number(event.target.value) || 0)}/></label><label>結束頁<input type="number" min="0" value={selected?.pdfEndPage || 0} onChange={(event) => update(active, "pdfEndPage", Number(event.target.value) || 0)}/></label></div><div><button onClick={markStart}>將第 {page} 頁設為起點</button><button onClick={markEnd}>將第 {page} 頁設為終點</button><button className="save" disabled={saving} onClick={() => void save()}>{saving ? "儲存中…" : "儲存全部對照"}</button></div></div>
      </section>
    </div>
  </section>;
}
