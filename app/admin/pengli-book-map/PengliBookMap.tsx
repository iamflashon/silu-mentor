  ...
  return <section className="pengli-map-shell">
    ...
    <div className="pengli-map-grid">
      ...
      <section className="pengli-map-workspace">
        ...
        {book.hasPdf ? <iframe key={page} title="私有 PDF 原稿" src={`/api/admin/pengli-book-map/source?id=${book.id}&locate=${page}#page=${page}&zoom=page-width&toolbar=0&navpanes=0&scrollbar=0`}/> : <div className="pengli-pdf-empty"><b>目前只有逐頁索引，尚未保存私有 PDF 原稿</b><p>上傳後只供管理後台核對，學生端不會取得 PDF。</p><label>{uploading ? "上傳中…" : "上傳受保護的 PDF 原稿"}<input type="file" accept="application/pdf,.pdf" disabled={uploading} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); }}/></label></div>}
