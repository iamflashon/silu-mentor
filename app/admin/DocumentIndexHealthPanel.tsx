...
export default function DocumentIndexHealthPanel() {
  ...
  async function scan(preserveProgress = false) {
    setScanning(true); if (!preserveProgress) setProgress([]);
    try {
      const response = await fetch("/api/admin/document-index-health", { cache: "no-store" });
      const payload = await response.json() as HealthPayload;
      if (!response.ok) throw new Error(payload.error || "索引健檢失敗");
      setData(payload); setSelected(payload.items.filter((item) => item.repairable).map((item) => item.id));
  ...
  async function repairSelected() {
    ...
    setProgress((rows) => [...rows, stopRef.current ? `已安全停止；完成 ${completed}/${targets.length} 份。` : `批次修復完成：${completed}/${targets.length} 份。`]);
  ...
  return <section className="panel index-health-panel">
    <header><div><p>INDEX HEALTH & REPAIR</p><h2>批次索引健檢與修復</h2><span>先檢查全部教材；修復完成前保留既有可用索引，只對可安全處理的項目接續補建。</span></div><button className="primary-btn" onClick={() => void scan()} disabled={scanning || repairing}>{scanning ? "掃描中…" : data ? "重新掃描全部教材" : "掃描全部教材"}</button></header>
