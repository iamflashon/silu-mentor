...
type Code = { id:string; last4:string; label:string; status:string; benefitType?:string; categories:string[]; productKey?:string; quota?:number; durationDays?:number; redeemBy:string|null; createdAt:string; redeemedAt:string|null; redeemedBy:string|null };
...
export default function AiAccessAdminPage(){
  const [policy,setPolicy]=useState<Policy|null>(null),[codes,setCodes]=useState<Code[]>([]),[notice,setNotice]=useState(""),[busy,setBusy]=useState(false),[count,setCount]=useState(10),[label,setLabel]=useState("培訓學生贈送"),[redeemBy,setRedeemBy]=useState(""),[generated,setGenerated]=useState<string[]>([]),[benefitType,setBenefitType]=useState("ai_access"),[codeDuration,setCodeDuration]=useState(30);
  ...
  async function generate(){const data=await post({action:"generate-codes",count,label,redeemBy:redeemBy||null,benefitType,durationDays:codeDuration,categories:policy?.categories??[]});if(data){setGenerated(data.generatedCodes??[]);setNotice(`已產生 ${data.generatedCodes?.length??0} 組一次性啟用碼。完整碼只在本次畫面顯示，請立即複製保存。`)}}
  ...
  return <main className="ai-access-admin">
    ...
    {!policy?<section className="ai-access-card">讀取方案設定中…</section>:<>
      ...
      <section className="ai-access-card"><div className="ai-access-section-title"><div><h2>免費一次性啟用碼</h2><p>提供培訓學生、老師、讀友或行銷試用；一碼限一個會員兌換一次。</p></div><span className="ai-access-count">{codes.length} 組紀錄</span></div><div className="ai-code-generator"><label>開通權益<select value={benefitType} onChange={event=>setBenefitType(event.target.value)}><option value="ai_access">AI 學習額度</option><option value="medtech_book">醫檢指定書籍</option></select></label><label>活動名稱<input value={label} onChange={event=>setLabel(event.target.value)}/></label><label>使用天數<input type="number" min="1" max="365" value={codeDuration} onChange={event=>setCodeDuration(Number(event.target.value))}/></label><label>產生數量<input type="number" min="1" max="100" value={count} onChange={event=>setCount(Number(event.target.value))}/></label><label>最晚兌換日（可留空）<input type="date" value={redeemBy} onChange={event=>setRedeemBy(event.target.value)}/></label><button type="button" onClick={()=>void generate()} disabled={busy}>產生啟用碼</button></div>
      ...
      <div className="ai-code-list">{codes.length?codes.slice(0,100).map(code=><article key={code.id}><div><b>{code.label}</b><span>••••-{code.last4} · {code.benefitType==="medtech_book"?`醫檢指定書籍 ${code.durationDays??30} 天`:`AI ${code.quota??policy.quota} 次／${code.durationDays??policy.durationDays} 天`}</span><small>建立 {new Date(code.createdAt).toLocaleString("zh-TW")}{code.redeemBy?` · ${code.redeemBy} 前兌換`:" · 無兌換截止日"}{code.redeemedBy?` · ${code.redeemedBy}`:""}</small></div><em className={`ai-code-${code.status}`}>{statusLabels[code.status]??code.status}</em>{code.status==="unused"&&<button type="button" onClick={()=>void disable(code.id)} disabled={busy}>停用</button>}</article>):<p>尚未產生啟用碼。</p>}</div></section>
    </>}
