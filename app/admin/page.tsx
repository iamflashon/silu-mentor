      ...
      <div className="admin-main">
        ...
        {activeTab === "external-index" && <section className="panel external-index-admin">
          ...
          {externalLoading ? <p className="usage-empty">正在讀取同步紀錄…</p> : (() => {
            ...
            return <div className="external-source-lists">{source.key === "get" && <div className="external-book-coverage"><header><div><b>高點圖書資料完整率</b><span>逐本詳細頁核對；缺欄位不算完成</span></div><strong>{books.length}<small> 本已辨識</small></strong></header><div><article><span>作者</span><b>{authorBooks}/{books.length}</b><em>{percent(authorBooks)}%</em></article><article><span>完整目錄</span><b>{catalogueBooks}/{books.length}</b><em>{percent(catalogueBooks)}%</em></article><article><span>書籍介紹</span><b>{descriptionBooks}/{books.length}</b><em>{percent(descriptionBooks)}%</em></article><article><span>整體達 80%</span><b>{completeBooks}/{books.length}</b><em>{percent(completeBooks)}%</em></article></div></div>}<section>
              ...
              {selected && <div className="external-index-detail"><div><span>目前層級</span><strong>第 {selected.depth ?? 1} 層</strong></div><div><span>資料類型</span><strong>{selected.book ? "書籍詳細資料" : selected.kind === "detail" ? "主題／內容" : "分類／入口"}</strong></div><div><span>上層來源</span><strong>{selected.parentTitle || source.label}</strong></div><div><span>下層資料</span><strong>{levelItems.length} 筆</strong></div>{selected.book?.authors?.length ? <div><span>作者</span><strong>{selected.book.authors.join("、")}</strong></div> : selected.teacher && <div><span>授課師資</span><strong>{selected.teacher}</strong></div>}{selected.book?.edition && <div><span>版次</span><strong>{selected.book.edition}</strong></div>}{selected.book?.publishedAt && <div><span>出版日期</span><strong>{selected.book.publishedAt}</strong></div>}{(selected.book?.isbn || selected.book?.bookCode) && <div><span>ISBN／書號</span><strong>{selected.book.isbn || selected.book.bookCode}</strong></div>}{selected.book && <div><span>資料完整度</span><strong>{selected.book.completeness ?? 0}%</strong></div>}<p>{selected.book?.description || selected.summary}</p>{selected.book?.catalogue?.length ? <details className="external-book-catalogue"><summary>查看完整目錄（{selected.book.catalogue.length} 項）</summary><ol>{selected.book.catalogue.map((row, index) => <li key={`${index}-${row}`}>{row}</li>)}</ol></details> : null}</div>}
              ...
              {selected && levelItems.length === 0 && <div className="external-index-leaf"><b>目前沒有可辨識的下一層公開資料</b><span>整體同步已自動往下探索；若來源網站稍後新增內容，按「重新同步」即可更新。右上角仍可單獨重抓此節點。</span></div>}
