...
type ExternalIndexSource = { id: number; key: "lawdata" | "get" | "ibrain"; label: string; sourceUrl: string; status: string; lastSyncedAt: string | null; items: Array<{ id: number; title: string; url: string; summary: string; enabled: boolean; indexed: boolean; accessType: string; depth?: number; parentTitle?: string; kind?: string; subject?: string; teacher?: string }> };
        ...
        {activeTab === "external-index" && <section className="panel external-index-admin">
          ...
          {externalLoading ? <p className="usage-empty">正在讀取同步紀錄…</p> : (() => {
            ...
            return <div className="external-source-lists"><section>
              ...
              {selected && <div className="external-index-detail"><div><span>目前層級</span><strong>第 {selected.depth ?? 1} 層</strong></div><div><span>資料類型</span><strong>{selected.kind === "detail" ? "主題／內容" : "分類／入口"}</strong></div><div><span>上層來源</span><strong>{selected.parentTitle || source.label}</strong></div><div><span>下層資料</span><strong>{levelItems.length} 筆</strong></div>{selected.subject && <div><span>對應科目</span><strong>{selected.subject}</strong></div>}{selected.teacher && <div><span>授課師資</span><strong>{selected.teacher}</strong></div>}<p>{selected.summary}</p></div>}
