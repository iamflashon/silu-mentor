...
type SubtitleSegment = { id: number; startSeconds: number; endSeconds: number; text: string; summary: string; importance: number; recommended: boolean; reviewStatus: string; sequence: number };
type ListeningItem = { id: number; questionId: number | null; title: string; year: string; subject: string; questionText: string; narrationScript: string; sourceUrl: string; audioStorageKey: string | null; audioFileName: string | null; status: string };
type ListeningSegment = { id: number; listeningId: number; fileName: string; durationSeconds: number; startOffsetSeconds: number; sequence: number };
type ListeningCue = { id: number; startSeconds: number; endSeconds: number; text: string; sequence: number };
type ExamQuestion = { id: number; examType: string; year: string; subject: string; questionNumber: string; stem: string; status: string; sourceUrl: string };
type EssayQuestion = { id: number; year: string; subject: string; questionNumber: string; stem: string; sourceUrl: string };
type LegalSource = { id: number; sourceKey: string; label: string; category: string; sourceUrl: string; status: string; documentCount: number; articleCount: number; importCursor: number; totalAvailable: number; lastError?: string | null; lastDownloadedAt?: string | null };
type JudicialStatus = { configured: boolean; caseCount: number; settings: Record<string, string> };
...
export default function AdminPage() {
  const [activeTab, setActiveTab] = useState<"documents" | "resources" | "courses" | "listening" | "magazine" | "legal" | "judicial" | "sources" | "questions" | "costs">("documents");
  ...
  const [subtitleCourse, setSubtitleCourse] = useState<LearningResource | null>(null);
  ...
  const [editingListening, setEditingListening] = useState<ListeningItem | null>(null);
  const [listeningSegments, setListeningSegments] = useState<ListeningSegment[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/listening").then(async (response) => { if (response.ok) { const result = await response.json() as { items?: ListeningItem[]; questions?: EssayQuestion[] }; setListeningItems(result.items ?? []); setEssayQuestions(result.questions ?? []); } }).catch(() => undefined);
    fetch("/api/legal-sources").then(async (response) => { if (response.ok) setLegalSources(((await response.json()) as { sources?: LegalSource[] }).sources ?? []); }).catch(() => undefined);
    fetch("/api/judicial-sync").then(async (response) => { if (response.ok) setJudicialStatus(await response.json() as JudicialStatus); }).catch(() => undefined);
  }, []);

  useEffect(() => { if (activeTab === "questions") loadExamQuestions(questionPage); }, [activeTab, questionPage]);

  async function syncLegal(sourceKey: string, restart = false) {
    setSyncingLegal(sourceKey); setNotice("正在下載官方資料並分批建立法規索引…"); const response = await fetch("/api/legal-sources", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sourceKey, restart }) }); const result = await readJson(response) as { status?: string; processed?: number; next?: number; total?: number; error?: string };
    setNotice(response.ok ? `本批完成 ${result.processed ?? 0} 筆；${result.status === "ready" ? "資料已可供 AI 搜尋" : `進度 ${result.next ?? 0} / ${result.total ?? 0}，請繼續下一批`}` : result.error ?? "資料同步失敗"); const refreshed = await fetch("/api/legal-sources"); if (refreshed.ok) setLegalSources(((await refreshed.json()) as { sources?: LegalSource[] }).sources ?? []); setSyncingLegal(null);
  }

  async function runJudicial(action: "test" | "sync") {
    setSyncingJudicial(true); setNotice(action === "test" ? "正在驗證司法院 API 帳密…" : "正在取得異動清單並下載本批裁判書…"); const response = await fetch("/api/judicial-sync", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, limit: 30 }) }); const result = await readJson(response) as { message?: string; imported?: number; pending?: number; removed?: number; error?: string }; setNotice(response.ok ? result.message ?? `下載 ${result.imported ?? 0} 筆、移除 ${result.removed ?? 0} 筆，尚待 ${result.pending ?? 0} 筆` : result.error ?? "司法院同步失敗"); const refreshed = await fetch("/api/judicial-sync"); if (refreshed.ok) setJudicialStatus(await refreshed.json() as JudicialStatus); setSyncingJudicial(false);
  }
  ...
  async function generateListening(event: FormEvent) {
    event.preventDefault(); setGeneratingListening(true); setNotice("AI 正在辨識題目並撰寫可直接配音的解題聞稿…");
    const form = new FormData(); form.set("questionId", listeningQuestionId); form.set("title", listeningTitle); form.set("questionText", listeningQuestionText); if (listeningFile) form.set("file", listeningFile);
    const response = await fetch("/api/listening", { method: "POST", body: form }); const result = await readJson(response) as { item?: ListeningItem; error?: string };
    if (!response.ok || !result.item) setNotice(result.error ?? "聞稿生成失敗"); else { setListeningItems((current) => [result.item!, ...current]); setEditingListening(result.item); setListeningQuestionId(""); setListeningTitle(""); setListeningQuestionText(""); setListeningFile(null); setNotice("聞稿已生成，請先校正內容，再上傳你的配音檔。"); }
    setGeneratingListening(false);
  }

  async function importPreparedListening() {
    if (!preparedTxt) return; const form = new FormData(); form.set("preparedTxt", preparedTxt); form.set("title", listeningTitle); form.set("questionText", listeningQuestionText); form.set("subject", "刑法"); setNotice("正在匯入自備 TXT 聞稿…");
    const response = await fetch("/api/listening", { method: "POST", body: form }); const result = await readJson(response) as { item?: ListeningItem; error?: string };
    if (!response.ok || !result.item) { setNotice(result.error ?? "TXT 匯入失敗"); return; } setListeningItems((current) => [result.item!, ...current]); setPreparedTxt(null); setListeningTitle(""); setListeningQuestionText(""); await openListeningEditor(result.item); setNotice("TXT 已匯入；現在可上傳一個完整音檔或多段音檔與 SRT。");
  }

  async function saveListening(item: ListeningItem, status = item.status) {
    const response = await fetch("/api/listening", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...item, status }) }); const result = await readJson(response) as { item?: ListeningItem; error?: string };
    if (!response.ok || !result.item) { setNotice(result.error ?? "儲存失敗"); return; } setListeningItems((current) => current.map((row) => row.id === item.id ? result.item! : row)); setEditingListening(result.item); setNotice(status === "published" ? "已發布為前台聽解題內容。" : "聞稿已儲存。");
  }

  async function uploadListeningAudio(item: ListeningItem, file?: File) {
    if (!file) return; const form = new FormData(); form.set("id", String(item.id)); form.set("audio", file); setNotice("正在上傳配音檔…"); const response = await fetch("/api/listening", { method: "PUT", body: form }); const result = await readJson(response) as { item?: ListeningItem; error?: string };
    if (!response.ok || !result.item) { setNotice(result.error ?? "配音上傳失敗"); return; } setListeningItems((current) => current.map((row) => row.id === item.id ? result.item! : row)); setEditingListening(result.item); setNotice("配音已上傳，可以試聽並發布。");
  }

  async function openListeningEditor(item: ListeningItem) {
    setEditingListening(item); const response = await fetch(`/api/listening/segments?listeningId=${item.id}`);
    if (response.ok) { const result = await response.json() as { segments?: ListeningSegment[]; cues?: ListeningCue[] }; setListeningSegments(result.segments ?? []); setListeningCues(result.cues ?? []); }
  }

  function audioDuration(file: File) {
    return new Promise<number>((resolve) => { const audio = new Audio(); const url = URL.createObjectURL(file); audio.preload = "metadata"; audio.onloadedmetadata = () => { URL.revokeObjectURL(url); resolve(Number.isFinite(audio.duration) ? audio.duration : 0); }; audio.onerror = () => { URL.revokeObjectURL(url); resolve(0); }; audio.src = url; });
  }

  async function uploadListeningSegments(files?: FileList | null) {
    if (!editingListening || !files?.length) return; setNotice(`正在依序上傳 ${files.length} 段配音…`);
    for (const file of Array.from(files)) { const form = new FormData(); form.set("action", "audio"); form.set("listeningId", String(editingListening.id)); form.set("file", file); form.set("durationSeconds", String(await audioDuration(file))); const response = await fetch("/api/listening/segments", { method: "POST", body: form }); if (!response.ok) { const result = await readJson(response); setNotice(String(result.error || `${file.name} 上傳失敗`)); return; } }
    await openListeningEditor(editingListening); setNotice("分段配音已依順序上傳，系統已建立連續時間軸。");
  }

  async function uploadListeningZip(file?: File) {
    if (!editingListening || !file) return; setNotice("正在解析 ZIP 並依檔名排列音檔…");
    try { const entries = unzipSync(new Uint8Array(await file.arrayBuffer())); const allAudio = Object.keys(entries).filter((name) => /\.(mp3|m4a|wav|aac|ogg)$/i.test(name)).sort((a, b) => a.localeCompare(b, "zh-TW", { numeric: true })); const numbered = allAudio.filter((name) => /^\d+\.(mp3|m4a|wav|aac|ogg)$/i.test(name.split("/").pop() || "")); const names = numbered.length ? numbered : allAudio; if (!names.length) { setNotice("ZIP 內找不到支援的音檔"); return; }
      for (const name of names) { const extension = name.split(".").pop()?.toLowerCase(); const type = extension === "m4a" ? "audio/mp4" : extension === "wav" ? "audio/wav" : extension === "ogg" ? "audio/ogg" : extension === "aac" ? "audio/aac" : "audio/mpeg"; const audioFile = new File([entries[name]], name.split("/").pop() || name, { type }); const form = new FormData(); form.set("action", "audio"); form.set("listeningId", String(editingListening.id)); form.set("file", audioFile); form.set("durationSeconds", String(await audioDuration(audioFile))); const response = await fetch("/api/listening/segments", { method: "POST", body: form }); if (!response.ok) throw new Error(audioFile.name); }
      const srtName = Object.keys(entries).find((name) => name.toLowerCase().endsWith(".srt")); if (srtName) await uploadListeningSrt(new File([entries[srtName]], srtName.split("/").pop() || "subtitles.srt", { type: "application/x-subrip" }));
      const txtNames = Object.keys(entries).filter((name) => name.toLowerCase().endsWith(".txt")); if (txtNames.length) { const decoder = new TextDecoder(); const candidates = txtNames.map((name) => ({ name, text: decoder.decode(entries[name]) })); const selected = candidates.sort((a, b) => ((b.text.match(/[為與臺條題應將]/g) || []).length - (a.text.match(/[為與臺條題應將]/g) || []).length))[0]; if (selected.text.trim() && window.confirm(`ZIP 內找到 ${txtNames.length} 份 TXT，是否以「${selected.name.split("/").pop()}」更新目前聞稿？`)) { const updated = { ...editingListening, narrationScript: selected.text.trim() }; await saveListening(updated, updated.status); } }
      await openListeningEditor(editingListening); setNotice(`ZIP 已完成：匯入 ${names.length} 段音檔${srtName ? "與 SRT 字幕" : ""}；已忽略重複的完整合併音檔。`);
    } catch { setNotice("ZIP 解析或分段上傳失敗，請確認檔案未加密且音檔格式正確。"); }
  }

  async function uploadListeningSrt(file?: File, segmentId?: number) {
    if (!editingListening || !file) return; const form = new FormData(); form.set("action", "subtitle"); form.set("listeningId", String(editingListening.id)); form.set("file", file); if (segmentId) form.set("segmentId", String(segmentId));
    const response = await fetch("/api/listening/segments", { method: "POST", body: form }); const result = await readJson(response); if (!response.ok) { setNotice(String(result.error || "字幕上傳失敗")); return; } await openListeningEditor(editingListening); setNotice(segmentId ? "此段 SRT 已加上音檔累計時間並完成對齊。" : `整份 SRT 已建立 ${result.cues ?? 0} 段字幕。`);
  }

  async function removeListeningSegment(id: number) { if (!editingListening || !window.confirm("確定移除此段音檔？")) return; await fetch(`/api/listening/segments?id=${id}`, { method: "DELETE" }); await openListeningEditor(editingListening); }
  async function applySubtitleOffset() { if (!editingListening || !subtitleOffset) return; await fetch("/api/listening/segments", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ listeningId: editingListening.id, offsetSeconds: subtitleOffset }) }); setSubtitleOffset(0); await openListeningEditor(editingListening); setNotice("字幕整體偏移已套用。"); }

  function downloadListeningTxt(item: ListeningItem) { const blob = new Blob([`${item.title}\n\n${item.questionText}\n\n【配音聞稿】\n${item.narrationScript}`], { type: "text/plain;charset=utf-8" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `${item.title.replace(/[\\/:*?"<>|]/g, "-")}.txt`; link.click(); URL.revokeObjectURL(url); }

  async function loadExamQuestions(page = questionPage) { const response = await fetch(`/api/exam-questions?page=${page}&status=draft`); if (!response.ok) return; const result = await response.json() as { items?: ExamQuestion[]; total?: number; totals?: Record<string, number> }; setExamQuestions(result.items ?? []); setQuestionTotal(result.total ?? 0); setQuestionTotals(result.totals ?? {}); }
  async function publishQuestions(ids?: number[], all = false) { const response = await fetch("/api/exam-questions", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(all ? { publishAllDrafts: true } : { ids, status: "published" }) }); const result = await readJson(response); setNotice(`已發布 ${result.updated ?? 0} 題，前台練真題現在可直接讀取。`); await loadExamQuestions(questionPage); }

  async function removeListening(item: ListeningItem) {
    if (!window.confirm(`確定移除「${item.title}」？`)) return; const response = await fetch(`/api/listening?id=${item.id}`, { method: "DELETE" }); if (!response.ok) return; setListeningItems((current) => current.filter((row) => row.id !== item.id)); if (editingListening?.id === item.id) setEditingListening(null); setNotice("聽解題項目已移除。");
  }
  ...
  async function editResource(resource: LearningResource) {
    const title = window.prompt("資源名稱", resource.title); if (title === null || !title.trim()) return;
    const creator = window.prompt("作者／老師／出版單位", resource.creator); if (creator === null) return;
    const sourceUrl = window.prompt("來源網址（書籍可留空）", resource.sourceUrl); if (sourceUrl === null) return;
    const response = await fetch("/api/resources", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...resource, title, creator, sourceUrl }) });
    const result = await readJson(response) as { resource?: LearningResource; error?: string };
    if (!response.ok || !result.resource) { setNotice(result.error ?? "資源編輯失敗"); return; }
    setResources((current) => current.map((item) => item.id === resource.id ? { ...item, ...result.resource } : item)); setNotice("資源資料已更新。");
  }

  async function removeResource(resource: LearningResource) {
    if (!window.confirm(`確定移除「${resource.title}」？相關字幕片段與書封也會刪除。`)) return;
    const response = await fetch(`/api/resources?id=${resource.id}`, { method: "DELETE" });
    const result = await readJson(response) as { error?: string };
    if (!response.ok) { setNotice(result.error ?? "資源移除失敗"); return; }
    setResources((current) => current.filter((item) => item.id !== resource.id)); setNotice(`${resource.title} 已移除。`);
  }
  ...
  async function openSubtitleEditor(resource: LearningResource) {
    ...
    setSubtitleCourse(resource); setSubtitleSegments(result.segments ?? []); setSegmentPage(1);
  }

  async function saveSegment(segment: SubtitleSegment) {
    const response = await fetch("/api/resources/segments", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...segment, reviewStatus: "reviewed" }) });
    const result = await response.json() as { segment?: SubtitleSegment };
    if (result.segment) setSubtitleSegments((current) => current.map((item) => item.id === segment.id ? result.segment! : item));
  }

  async function analyzeCourseSegments() {
    if (!subtitleCourse) return; setAnalyzingSegments(true);
    const response = await fetch("/api/resources/segments", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ resourceId: subtitleCourse.id }) });
    const result = await readJson(response) as { analyzed?: number; error?: string };
    if (!response.ok) setNotice(result.error ?? "AI 重點分析失敗"); else { setNotice(`AI 已分析 ${result.analyzed ?? 0} 個字幕片段。`); await openSubtitleEditor(subtitleCourse); }
    ...
    <main className="admin-shell">
      ...
      <div className="admin-main">
        ...
        <nav className="admin-tabs" aria-label="後台功能切換">
          ...
          <button className={activeTab === "listening" ? "active" : ""} onClick={() => setActiveTab("listening")}>聽解題</button>
          <button className={activeTab === "magazine" ? "active" : ""} onClick={() => setActiveTab("magazine")}>月旦法學教室</button>
          <button className={activeTab === "legal" ? "active" : ""} onClick={() => setActiveTab("legal")}>法規與憲法法庭</button>
          ...
          <button className={activeTab === "questions" ? "active" : ""} onClick={() => setActiveTab("questions")}>真題審核</button>
        ...
        {(activeTab === "resources" || activeTab === "courses") && <section className="panel resource-manager">
          ...
          <div className="resource-grid">{resources.filter((resource) => activeTab === "courses" ? resource.resourceType === "course" : resource.resourceType === "book").map((resource) => <article className="resource-card" key={resource.id}>
            ...
            <div className="resource-actions">{resource.resourceType === "book" && <select aria-label={`${resource.title}綁定教材 PDF`} value={resource.documentId ?? ""} onChange={(e) => bindBookDocument(resource, e.target.value)}><option value="">選擇教材 PDF</option>{files.map((file) => <option key={file.id} value={file.id}>{file.name}</option>)}</select>}<label>上傳書封<input type="file" accept="image/*" hidden onChange={(e) => uploadResourceAsset(resource.id, "cover", e.target.files?.[0])} /></label>{resource.resourceType === "course" && <><label>上傳 SRT<input type="file" accept=".srt" hidden onChange={(e) => uploadResourceAsset(resource.id, "subtitle", e.target.files?.[0])} /></label><button type="button" className="subtitle-open" onClick={() => openSubtitleEditor(resource)}>校正字幕／重點</button></>}<div className="resource-edit-actions"><button type="button" onClick={() => editResource(resource)}>編輯</button><button type="button" className="danger" onClick={() => removeResource(resource)}>移除</button></div></div>
          </article>)}</div>
        </section>}
        {activeTab === "listening" && <section className="panel listening-manager"><div className="cost-heading"><div><h2>解題書用聽的</h2><p className="panel-sub">AI 生成聞稿後，可依序上傳多段音檔，再上傳整份或各段 SRT；系統會自動接成同一條時間軸。</p></div><span className="source-count">{listeningItems.length} 篇</span></div><form className="listening-create" onSubmit={generateListening}><label className="field">從二試真題庫選題<select value={listeningQuestionId} onChange={(e) => setListeningQuestionId(e.target.value)}><option value="">不選，改用貼上／上傳</option>{essayQuestions.map((question) => <option value={question.id} key={question.id}>{question.year} · {question.subject} · 第 {question.questionNumber} 題</option>)}</select></label><label className="field">節目標題（可由 AI 產生）<input value={listeningTitle} onChange={(e) => setListeningTitle(e.target.value)} placeholder="例如：刑法二試｜共同正犯與因果歷程" /></label><label className="field listening-question">直接貼上題目<textarea value={listeningQuestionText} onChange={(e) => setListeningQuestionText(e.target.value)} rows={5} placeholder="貼上申論題題幹；若已選真題可留空" /></label><label className="listening-upload">上傳題目圖片或 PDF<input type="file" accept="image/*,.pdf" hidden onChange={(e) => setListeningFile(e.target.files?.[0] ?? null)} /><strong>{listeningFile?.name || "選擇題目檔"}</strong><span>圖片／PDF，12MB 以下</span></label><button className="primary-btn" disabled={generatingListening || (!listeningQuestionId && !listeningQuestionText.trim() && !listeningFile)}>{generatingListening ? "AI 正在生成聞稿…" : "AI 生成解題聞稿"}</button></form>{notice && <div className="notice">{notice}</div>}<div className="listening-list">{listeningItems.map((item) => <article key={item.id} className={item.status === "published" ? "published" : ""}><div className="listening-badge">聽</div><div><span>{item.year || "自訂題目"} · {item.subject}</span><h3>{item.title}</h3><p>{item.narrationScript.slice(0, 90)}…</p><small>{item.status === "published" ? "前台發布" : "草稿"}</small></div><div className="listening-actions"><button onClick={() => openListeningEditor(item)}>校稿／分段配音</button><button className="danger" onClick={() => removeListening(item)}>移除</button></div></article>)}</div></section>}
        {activeTab === "magazine" && <section className="panel resource-manager"><div className="cost-heading"><div><h2>月旦法學教室</h2><p className="panel-sub">貼入歷期網址後，自動找到最新一期並分析出刊資料、試讀文章與合法連結；資料先進草稿，確認後再供前台推薦。</p></div><span className="source-count">{resources.filter((item) => item.resourceType === "magazine").length} 期</span></div><div className="magazine-import"><label className="field">月旦法學教室歷期網址<input type="url" value={magazineUrl} onChange={(e) => setMagazineUrl(e.target.value)} /></label><button type="button" className="primary-btn" onClick={analyzeMagazine}>自動分析最新一期</button></div>{notice && <div className="notice">{notice}</div>}<div className="resource-grid">{resources.filter((item) => item.resourceType === "magazine").map((resource) => <article className="resource-card" key={resource.id}><div className="resource-cover"><span>刊</span></div><div className="resource-info"><span>{resource.status === "draft" ? "待確認" : "前台顯示"}</span><h3>{resource.title}</h3><p>{resource.creator}</p><small>{resource.description || "尚未取得出刊資料"} · {resource.segmentCount} 篇內容</small></div><div className="resource-actions"><a href={resource.sourceUrl} target="_blank" rel="noreferrer">檢視來源</a><div className="resource-edit-actions"><button type="button" onClick={() => editResource(resource)}>編輯</button><button type="button" className="danger" onClick={() => removeResource(resource)}>移除</button></div></div></article>)}</div></section>}
        ...
        {activeTab === "questions" && <section className="panel question-review"><div className="cost-heading"><div><h2>真題拆解審核</h2><p className="panel-sub">拆解後預設為草稿；發布後才會出現在前台的一試選擇題與二試申論題。</p></div><span className="source-count">草稿 {questionTotals.draft ?? questionTotal} · 已發布 {questionTotals.published ?? 0}</span></div><div className="question-review-actions"><button type="button" disabled={!examQuestions.length} onClick={() => publishQuestions(examQuestions.map((item) => item.id))}>發布本頁 10 題</button><button type="button" className="primary-btn" disabled={!questionTotal} onClick={() => window.confirm(`確定發布全部 ${questionTotal} 題草稿？`) && publishQuestions(undefined, true)}>批次發布全部草稿</button></div>{notice && <div className="notice">{notice}</div>}<div className="question-review-list">{examQuestions.map((question) => <article key={question.id}><header><span>{question.examType === "mcq" ? "一試選擇題" : "二試申論題"}</span><b>{question.year} · {question.subject} · 第 {question.questionNumber} 題</b></header><p>{question.stem}</p><footer>{question.sourceUrl && <a href={question.sourceUrl} target="_blank" rel="noreferrer">檢視來源</a>}<button onClick={() => publishQuestions([question.id])}>發布前台</button></footer></article>)}</div>{questionTotal > 10 && <nav className="document-pagination"><button disabled={questionPage === 1} onClick={() => setQuestionPage((page) => page - 1)}>上一頁</button><span>第 {questionPage} / {Math.ceil(questionTotal / 10)} 頁</span><button disabled={questionPage >= Math.ceil(questionTotal / 10)} onClick={() => setQuestionPage((page) => page + 1)}>下一頁</button></nav>}</section>}
      </div>
      {subtitleCourse && <div className="subtitle-editor-backdrop"><section className="subtitle-editor"><header><div><span>字幕校正與重點摘要</span><h2>{subtitleCourse.title}</h2></div><button onClick={() => setSubtitleCourse(null)}>×</button></header><div className="subtitle-workspace"><div className="course-reference"><iframe src={subtitleCourse.sourceUrl} title={`${subtitleCourse.title}課程畫面`} /><a href={subtitleCourse.sourceUrl} target="_blank" rel="noreferrer">若畫面無法嵌入，另開課程頁對照 ↗</a><button onClick={analyzeCourseSegments} disabled={analyzingSegments}>{analyzingSegments ? "AI 分析中，請稍候…" : "AI 自動拆解推薦重點"}</button><p>AI 會提出重要度與摘要，管理員確認後才供前台推薦。</p></div><div className="subtitle-list">{subtitleSegments.slice((segmentPage - 1) * 10, segmentPage * 10).map((segment) => <article key={segment.id} className={segment.recommended ? "recommended" : ""}><div className="segment-time"><input type="number" value={segment.startSeconds} onChange={(e) => setSubtitleSegments((current) => current.map((item) => item.id === segment.id ? { ...item, startSeconds: Number(e.target.value) } : item))} /><span>至</span><input type="number" value={segment.endSeconds} onChange={(e) => setSubtitleSegments((current) => current.map((item) => item.id === segment.id ? { ...item, endSeconds: Number(e.target.value) } : item))} /><a href={`${subtitleCourse.sourceUrl}#t=${segment.startSeconds}`} target="_blank" rel="noreferrer">跳到此段</a></div><textarea value={segment.text} onChange={(e) => setSubtitleSegments((current) => current.map((item) => item.id === segment.id ? { ...item, text: e.target.value } : item))} /><input className="segment-summary" value={segment.summary} placeholder="重點摘要（AI 分析或人工填寫）" onChange={(e) => setSubtitleSegments((current) => current.map((item) => item.id === segment.id ? { ...item, summary: e.target.value } : item))} /><footer><label>重要度<select value={segment.importance} onChange={(e) => setSubtitleSegments((current) => current.map((item) => item.id === segment.id ? { ...item, importance: Number(e.target.value) } : item))}>{[0,1,2,3,4,5].map((value) => <option key={value}>{value}</option>)}</select></label><label><input type="checkbox" checked={segment.recommended} onChange={(e) => setSubtitleSegments((current) => current.map((item) => item.id === segment.id ? { ...item, recommended: e.target.checked } : item))} /> 前台推薦</label><button onClick={() => saveSegment(segment)}>儲存校正</button></footer></article>)}</div></div><nav className="document-pagination"><button disabled={segmentPage === 1} onClick={() => setSegmentPage((page) => page - 1)}>上一頁</button><span>第 {segmentPage} / {Math.max(1, Math.ceil(subtitleSegments.length / 10))} 頁</span><button disabled={segmentPage >= Math.ceil(subtitleSegments.length / 10)} onClick={() => setSegmentPage((page) => page + 1)}>下一頁</button></nav></section></div>}
      {editingListening && <div className="subtitle-editor-backdrop"><section className="listening-editor"><header><div><span>聽解題聞稿、分段配音與字幕</span><h2>{editingListening.title}</h2></div><button onClick={() => setEditingListening(null)}>×</button></header><label>標題<input value={editingListening.title} onChange={(e) => setEditingListening({ ...editingListening, title: e.target.value })} /></label><label>原始題目<textarea rows={5} value={editingListening.questionText} onChange={(e) => setEditingListening({ ...editingListening, questionText: e.target.value })} /></label><label>配音聞稿<textarea className="narration-editor" rows={14} value={editingListening.narrationScript} onChange={(e) => setEditingListening({ ...editingListening, narrationScript: e.target.value })} /></label><div className="listening-segment-tools"><button type="button" onClick={() => downloadListeningTxt(editingListening)}>下載聞稿 TXT</button><label>依序上傳多段音檔<input type="file" multiple accept="audio/*,.mp3,.m4a,.wav" hidden onChange={(e) => uploadListeningSegments(e.target.files)} /></label><label>上傳整份 SRT<input type="file" accept=".srt" hidden onChange={(e) => uploadListeningSrt(e.target.files?.[0])} /></label><div><input type="number" value={subtitleOffset} onChange={(e) => setSubtitleOffset(Number(e.target.value))} aria-label="字幕偏移秒數" /><button type="button" onClick={applySubtitleOffset}>字幕整體偏移</button></div></div><div className="audio-segment-list">{listeningSegments.map((segment) => <article key={segment.id}><b>第 {segment.sequence + 1} 段 · {segment.fileName}</b><span>{segment.startOffsetSeconds}s–{segment.startOffsetSeconds + segment.durationSeconds}s</span><audio controls preload="none" src={`/api/listening/segments/audio?id=${segment.id}`} /><label>此段 SRT<input type="file" accept=".srt" hidden onChange={(e) => uploadListeningSrt(e.target.files?.[0], segment.id)} /></label><button type="button" onClick={() => removeListeningSegment(segment.id)}>移除</button></article>)}</div><p className="subtitle-summary">{listeningSegments.length} 段音檔 · {listeningCues.length} 段字幕。整份 SRT 使用合併後時間軸；各段 SRT 可從 00:00 開始，系統會自動加上該段起始時間。</p><footer><button onClick={() => saveListening(editingListening, "draft")}>儲存草稿</button><button className="publish-listening" disabled={!listeningSegments.length} onClick={() => saveListening(editingListening, "published")}>發布聽解題</button></footer></section></div>}
      {activeTab === "legal" && <section className="panel data-hub"><div className="cost-heading"><div><h2>法規與憲法法庭資料</h2><p className="panel-sub">每個官方來源獨立下載與管理；完成解析後才供 AI 導師引用。</p></div><span className="source-count">{legalSources.reduce((sum, item) => sum + item.documentCount, 0).toLocaleString()} 筆</span></div>{notice && <div className="notice">{notice}</div>}<div className="data-source-grid">{legalSources.map((source) => <article key={source.sourceKey}><header><span>{source.category}</span><em className={`data-status ${source.status}`}>{source.status === "ready" ? "可供搜尋" : source.status === "importing" ? "匯入中" : source.status === "failed" ? "失敗" : source.status === "downloading" ? "下載中" : "尚未下載"}</em></header><h3>{source.label}</h3><p>{source.sourceKey.startsWith("moj-") ? "法務部官方開放資料，每月更新" : "司法院憲法法庭官方資料"}</p><div className="data-metrics"><div><b>{source.documentCount.toLocaleString()}</b><small>文件</small></div><div><b>{source.articleCount.toLocaleString()}</b><small>條文</small></div><div><b>{source.totalAvailable ? `${source.importCursor || source.totalAvailable}/${source.totalAvailable}` : "—"}</b><small>批次進度</small></div></div>{source.lastError && <small className="data-error">{source.lastError}</small>}<footer><a href={source.sourceUrl} target="_blank" rel="noreferrer">官方來源</a><button disabled={syncingLegal !== null} onClick={() => syncLegal(source.sourceKey, source.status === "ready")}>{syncingLegal === source.sourceKey ? "處理中…" : source.status === "importing" ? "繼續下一批" : source.status === "ready" ? "重新同步" : "開始下載"}</button></footer></article>)}</div></section>}{activeTab === "judicial" && <section className="panel judicial-hub"><div className="cost-heading"><div><h2>司法院裁判資料</h2><p className="panel-sub">使用已儲存帳密取得 6 小時 Token；官方 API 僅於每日 00:00 至 06:00 開放。</p></div><span className={`source-count ${judicialStatus?.configured ? "configured" : ""}`}>{judicialStatus?.configured ? "帳密已設定" : "尚未設定"}</span></div>{notice && <div className="notice">{notice}</div>}<div className="judicial-overview"><article><span>裁判資料庫</span><strong>{Number(judicialStatus?.caseCount ?? 0).toLocaleString()}</strong><small>已保存裁判</small></article><article><span>今晚排程</span><strong>00:30</strong><small>官方服務時間內執行</small></article><article><span>待下載</span><strong>{Number(judicialStatus?.settings?.judicial_pending_count ?? 0).toLocaleString()}</strong><small>每批 30 筆續傳</small></article></div><div className="judicial-actions"><button onClick={() => runJudicial("test")} disabled={syncingJudicial || !judicialStatus?.configured}>測試 API 帳密</button><button className="primary-btn" onClick={() => runJudicial("sync")} disabled={syncingJudicial || !judicialStatus?.configured}>{syncingJudicial ? "同步中…" : "立即下載一批"}</button></div><div className="sync-log"><h3>同步狀態</h3><p>最後驗證：{judicialStatus?.settings?.judicial_last_auth_at ? new Date(judicialStatus.settings.judicial_last_auth_at).toLocaleString("zh-TW") : "尚未驗證"}</p><p>最後同步：{judicialStatus?.settings?.judicial_last_sync_at ? new Date(judicialStatus.settings.judicial_last_sync_at).toLocaleString("zh-TW") : "尚未同步"}</p><p>{judicialStatus?.settings?.judicial_last_sync_summary || "今晚首次同步後會顯示下載與移除筆數。"}</p>{judicialStatus?.settings?.judicial_last_error && <p className="data-error">{judicialStatus.settings.judicial_last_error}</p>}</div></section>}
      {activeTab === "listening" && !editingListening && <section className="panel prepared-listening-import"><h3>已有完成的聞稿</h3><p>直接匯入 TXT，不呼叫 AI；建立後可上傳完整音檔、分段音檔或套件 ZIP。</p><div><input value={listeningTitle} onChange={(e) => setListeningTitle(e.target.value)} placeholder="節目標題（可留空使用 TXT 檔名）" /><label>選擇 TXT<input type="file" accept=".txt,text/plain" hidden onChange={(e) => setPreparedTxt(e.target.files?.[0] ?? null)} /></label><button disabled={!preparedTxt} onClick={importPreparedListening}>匯入 {preparedTxt?.name || "TXT"}</button></div></section>}
      {editingListening && <div className="zip-import-float"><label>匯入套件 ZIP<input type="file" accept=".zip,application/zip" hidden onChange={(e) => uploadListeningZip(e.target.files?.[0])} /></label></div>}
    </main>
  );
}
