    ...
    <main className="admin-shell">
      ...
      <div className="admin-main">
        ...
        {activeTab === "external-index" && <section className="panel external-index-admin">
          ...
          <div className="external-source-tabs" role="tablist" aria-label="資源網站">{(["lawdata", "get", "ibrain"] as const).map((key) => { const config = key === "lawdata" ? { label: "元照／月旦", note: "期刊、文章、書籍、講座、課程、活動與公開試閱" } : key === "get" ? { label: "高點出版", note: "司律書籍與目錄" } : { label: "iBrain 知識達", note: "司律課程與試聽" }; const source = externalSources.find((item) => item.key === key); return <button type="button" role="tab" aria-selected={externalSourceTab === key} className={externalSourceTab === key ? "active" : ""} key={key} onClick={() => { setExternalSourceTab(key); setExternalPage(1); setExternalQuery(""); }}><span><b>{config.label}</b><small>{config.note}</small></span><strong>{source?.items.length ?? 0}<small> 筆</small></strong></button>; })}</div>
          {externalNotice && <p className="external-index-notice">{externalNotice}</p>}
          {externalLoading ? <p className="usage-empty">正在讀取同步紀錄…</p> : (() => { const source = externalSources.find((item) => item.key === externalSourceTab); if (!source) return <div className="external-index-empty"><b>這個來源尚未建立索引</b><span>按「開始同步」抓取公開資料，供首頁跨來源推薦。</span><button disabled={externalSyncing !== "" || externalDeleting !== ""} onClick={() => void syncExternalSource(externalSourceTab)}>{externalSyncing === externalSourceTab ? "同步中…" : "開始同步"}</button></div>; const filtered = source.items.filter((item) => !externalQuery.trim() || `${source.label} ${item.title} ${item.summary}`.toLowerCase().includes(externalQuery.trim().toLowerCase())); const pageCount = Math.max(1, Math.ceil(filtered.length / 10)); const page = Math.min(externalPage, pageCount); const rows = filtered.slice((page - 1) * 10, page * 10); return <div className="external-source-lists"><section><header><div><h3>{source.label}</h3><span>{source.items.filter((item) => item.enabled).length} 筆啟用／{source.items.length} 筆已抓取</span></div><div className="external-source-actions"><a href={source.sourceUrl} target="_blank" rel="noreferrer">查看來源 ↗</a><button className="danger" disabled={externalSyncing !== "" || externalDeleting !== "" || source.items.length === 0} onClick={() => void deleteExternalSource(source)}>{externalDeleting === source.key ? "清除中…" : "清除此來源舊資料"}</button><button disabled={externalSyncing !== "" || externalDeleting !== ""} onClick={() => void syncExternalSource(source.key)}>{externalSyncing === source.key ? "同步中…" : "重新同步"}</button></div></header><div className="external-index-table"><div className="external-index-row table-head"><span>資源名稱</span><span>權限</span><span>首頁索引</span><span>使用</span></div>{rows.map((item) => <div className="external-index-row" key={item.id}><div><a href={item.url} target="_blank" rel="noreferrer">{item.title}</a><small>{item.summary}</small></div><span><em>公開索引</em></span><span className={item.indexed ? "indexed" : "disabled"}>{item.indexed ? "已索引" : "已停用"}</span><label className="external-index-toggle"><input type="checkbox" checked={item.enabled} onChange={(event) => void toggleExternalItem(item.id, event.target.checked)} /><span>{item.enabled ? "啟用" : "停用"}</span></label></div>)}</div>{filtered.length === 0 ? <p className="usage-empty">這個來源沒有符合搜尋條件的資料。</p> : <nav className="external-pagination" aria-label="資源分頁"><span>第 {page}／{pageCount} 頁，共 {filtered.length} 筆</span><div><button disabled={page <= 1} onClick={() => setExternalPage(page - 1)}>上一頁</button>{Array.from({ length: pageCount }, (_, index) => index + 1).map((number) => <button key={number} className={number === page ? "active" : ""} onClick={() => setExternalPage(number)}>{number}</button>)}<button disabled={page >= pageCount} onClick={() => setExternalPage(page + 1)}>下一頁</button></div></nav>}</section></div>; })()}
        </section>}
