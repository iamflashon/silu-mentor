...
type ExternalRetrievalTest = { query: string; found: boolean; complete: boolean; failureReason: string; target: { id: number; title: string; enabled: boolean; indexed: boolean; parentTitle: string }; matches: Array<{ id: number; source: string; title: string; summary: string; parentTitle: string; depth: number; enabled: boolean; indexed: boolean; excerpt: string }> };
...
export default function AdminPage() {
  ...
  async function testExternalHomepageRetrieval(item: ExternalIndexSource["items"][number]) {
    ...
    try {
      ...
      setExternalNotice(data.complete ? `首頁可以找到「${item.title}」及其實際文章。` : data.found ? `首頁目前只能找到「${item.title}」本身，尚未完整命中文章。` : `首頁目前找不到「${item.title}」。`);
