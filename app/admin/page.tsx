...
export default function AdminPage() {
  const [activeTab, setActiveTab] = useState<"documents" | "resources" | "courses" | "listening" | "magazine" | "legal" | "judicial" | "sources" | "costs">("documents");
  ...
  return (
    <main className="admin-shell">
      ...
      {editingListening && <div className="subtitle-editor-backdrop"><section className="listening-editor"><header><div><span>聽解題聞稿校正</span><h2>{editingListening.title}</h2></div><button onClick={() => setEditingListening(null)}>×</button></header><label>標題<input value={editingListening.title} onChange={(e) => setEditingListening({ ...editingListening, title: e.target.value })} /></label><label>原始題目<textarea rows={7} value={editingListening.questionText} onChange={(e) => setEditingListening({ ...editingListening, questionText: e.target.value })} /></label><label>配音聞稿<textarea className="narration-editor" rows={20} value={editingListening.narrationScript} onChange={(e) => setEditingListening({ ...editingListening, narrationScript: e.target.value })} /></label>{editingListening.audioStorageKey && <audio controls src={`/api/listening/audio?id=${editingListening.id}`} />}<footer><button onClick={() => saveListening(editingListening, "draft")}>儲存草稿</button><label>上傳配音<input type="file" accept="audio/*,.mp3,.m4a,.wav" hidden onChange={(e) => uploadListeningAudio(editingListening, e.target.files?.[0])} /></label><button className="publish-listening" disabled={!editingListening.audioStorageKey} onClick={() => saveListening(editingListening, "published")}>發布聽解題</button></footer></section></div>}
