...
export default function AdminLoginPage() {
  ...
  return <main className="main-entry-gate">
    <section className="admin-login-card">
      ...
      <h1>管理員登入</h1>
      <p>請輸入管理員密碼，驗證後才能進入司律備考與醫檢師平台。</p>
      <form className="admin-login-form" onSubmit={submit}>
        <label htmlFor="admin-entry-password">管理員密碼</label>
        <input id="admin-entry-password" type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} required autoFocus />
