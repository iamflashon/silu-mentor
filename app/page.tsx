...
type AssistantMode = "advisor" | "coach";
...
function citationStatusLabel(status?: string) {
  ...
  if (status === "official_web") return "專業查證：已取得官方來源並核對";
  if (status === "secondary_web") return "專業查證：可信裁判索引已命中，官方原文仍應覆核";
  if (status === "verified_cache") return "專業查證：使用30日內已核對的同類命題資料";
  return "引用狀態：未取得可核對教材";
}
function sourceDetails(source: string) {
  const separator = source.lastIndexOf("｜");
  const candidateUrl = separator >= 0 ? source.slice(separator + 1).trim() : "";
  const url = /^https?:\/\//i.test(candidateUrl) ? candidateUrl : "";
  const label = url ? source.slice(0, separator).trim() : source.trim();
  return { label: sourceDisplayName(label), url };
}
...
function visibleSourceNames(sources?: string[]) {
  return [...new Set((sources ?? []).map((source) => sourceDetails(source).label).filter(Boolean))];
}
function SourceLinks({ sources }: { sources?: string[] }) {
  const unique = [...new Map((sources ?? []).map((source) => {
    const detail = sourceDetails(source);
    return [detail.url || detail.label, detail];
  })).values()].filter((item) => item.label);
  if (!unique.length) return null;
  return <>{unique.map((item, index) => <span key={`${item.url || item.label}-${index}`}>{index ? "、" : ""}{item.url ? <a href={item.url} target="_blank" rel="noreferrer">{item.label}</a> : item.label}</span>)}</>;
}
  ...
  return <section className="model-comparison-card" aria-label="AI 模型測試比較">
    ...
    <div className="model-comparison-grid">
      {comparison.responses.map((response) => <article className={`model-comparison-response ${selectedKeys.includes(`teacher:${messageIndex}:${response.id}:${response.label}`) ? "follow-up-selected" : ""}`} key={response.id}>
        ...
        {visibleSourceNames(response.sources).length > 0 && <small className="model-comparison-sources">查證來源：<SourceLinks sources={response.sources} /></small>}
...
export function LawHome() {
  ...
  useEffect(() => {
    ...
    const saved = window.localStorage.getItem("ibrain-assistant-mode");
    setAssistantMode(requested === "advisor" || requested === "coach" ? requested : saved === "advisor" ? "advisor" : "coach");
  }, []);
  function chooseAssistantMode(mode: AssistantMode) {
    setAssistantMode(mode);
    window.localStorage.setItem("ibrain-assistant-mode", mode);
    if (!input.trim()) setInput(mode === "advisor" ? "請先了解我的考試目標與目前程度，再推薦適合的高點學習資源與讀書順序。" : "請陪我檢查今天的進度，並從最需要加強的地方開始。" );
  ...
  async function send(text: string, overrideMode?: ChatModelMode, options?: { hideStudentMessage?: boolean; professionalVerification?: boolean }) {
    ...
    try {
      ...
      const response = await fetch("/api/chat", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ messages: requestMessages.slice(-12), sessionId: activeSessionId, imageDataUrl: attachedImage, modelMode: overrideMode ?? modelMode, teachingLevel: sentTeachingLevel, assistantMode, persistStudentMessage: !options?.hideStudentMessage, professionalVerification: options?.professionalVerification === true, requestKey }),
      });
    ...
    <main className={`coach-shell ${chatFocusMode ? "chat-focus-mode" : ""}`}>
      ...
      <div className={`composer-wrap rail-${railSide} ${railCollapsed ? "rail-collapsed" : ""}`}>
        ...
        >
          ...
          <section className="assistant-mode-switch" aria-label="選擇 AI 服務模式">
            <button type="button" className={assistantMode === "advisor" ? "active" : ""} aria-pressed={assistantMode === "advisor"} onClick={() => chooseAssistantMode("advisor")}><span>⌕</span><b>考試顧問</b><small>找教材・排讀書路線</small></button>
            <button type="button" className={assistantMode === "coach" ? "active" : ""} aria-pressed={assistantMode === "coach"} onClick={() => chooseAssistantMode("coach")}><span>◎</span><b>陪考教練</b><small>陪練・追蹤進度</small></button>
          </section>
          {aiMeter?.active&&<div className="ai-meter-row"><a className="ai-usage-meter" href="/account#ai-access" aria-label={`AI 教練進度 ${aiMeter.coachRoundsUsed}／${aiMeter.coachRoundsTarget} 輪，剩餘 ${aiMeter.remaining} 次`}><strong>{assistantMode === "advisor" ? "考試顧問" : "陪考教練"} {aiMeter.coachRoundsUsed}／{aiMeter.coachRoundsTarget} 輪</strong><span>{practiceCoaching||thinking?"AI 回覆完成後計入本輪":aiMeter.coachRoundsUsed===0?`再完成 ${aiMeter.coachRoundsTarget} 輪扣 1 次`:`再完成 ${Math.max(0,aiMeter.coachRoundsTarget-aiMeter.coachRoundsUsed)} 輪扣 1 次`} · 剩餘 {aiMeter.remaining} 次</span><em>查看方案</em></a><button className="professional-verification-button" type="button" onClick={() => setVerificationOpen(true)} disabled={thinking || practiceCoaching || !input.trim() || aiMeter.coachWebSearchUsed>=1} title={aiMeter.coachWebSearchUsed>=1?"本組手動查證已使用；重大實務問題仍會自動查證":input.trim()?"主動查證目前輸入的最新法律資料":"請先輸入要查證的問題"}>{aiMeter.coachWebSearchUsed>=1?"重大實務仍自動查證":"查證最新資料"}</button></div>}
        ...
        <form className="composer" onSubmit={submit} onPaste={(event) => { const image = Array.from(event.clipboardData.items).find((item) => item.type.startsWith("image/"))?.getAsFile(); if (image) { event.preventDefault(); chooseQuestionImage(new File([image], `貼上的題目-${Date.now()}.png`, { type: image.type })); } }}>
          ...
          <textarea
            ...
            placeholder={practiceQuestion && practiceDiscussion ? "針對本題自由追問；AI 會直接回答，不會再反問" : practiceQuestion && practiceCoachMessages.length > 0 ? "回答教練的問題；不知道也可以說卡在哪裡" : assistantMode === "advisor" ? "告訴我目標考試、目前程度，或要找哪一類高點資源……" : "告訴我今天讀到哪裡，或直接貼上一道題目……"}
            value={input}
      ...
      {verificationOpen && <div className="professional-verification-backdrop" role="dialog" aria-modal="true" aria-labelledby="professional-verification-title"><section className="professional-verification-dialog"><div className="professional-verification-heading"><span>PROFESSIONAL LEGAL VERIFICATION</span><h2 id="professional-verification-title">AI 專業法學查證</h2><p>不是代替 Google，而是把可追溯的法律資料整理成可用於考試的答案。</p></div><div className="professional-verification-value"><article><b>① 重大問題自動查證</b><span>程序准駁、實務見解與裁判字號會自動升級Sol</span></article><article><b>② 官方來源優先</b><span>司法院、憲法法庭、全國法規資料庫及政府機關</span></article><article><b>③ 可信索引補充</b><span>官方搜尋未命中時，再查裁判索引並清楚標示來源層級</span></article><article><b>④ 可追溯結果</b><span>顯示裁判字號、來源連結、核對狀態與查證日期</span></article></div><div className="professional-verification-notice"><strong>重大實務問題不必手動按：系統會自動查證</strong><span>本按鈕保留給修法、最新資料或你想主動覆核的問題；每組5輪可另外手動使用1次。</span></div><div className="professional-verification-actions"><button type="button" className="secondary" onClick={() => setVerificationOpen(false)}>回到一般教材回答</button><button type="button" onClick={() => { const question=input.trim(); setVerificationOpen(false); if(question) void send(question,undefined,{professionalVerification:true}); }} disabled={!input.trim() || thinking}>立即進行完整查證</button></div></section></div>}
...
export function MainEntryGate() {
  ...
  return <main className="main-entry-gate main-portal">
    ...
    <section className="main-portal-shell">
      ...
      <section className="main-ai-entry" aria-labelledby="main-ai-entry-title">
        <div className="main-ai-entry-copy">
          <span>YOUR EXAM PARTNER</span>
          <h2 id="main-ai-entry-title">先問 AI，再開始今天的備考</h2>
          <p>從高點已授權與平台已匯入的資源找方向，或讓教練接續你的讀書進度。</p>
          <Link className="main-ai-own-chatgpt" href="/mcp/connect">使用自己的 ChatGPT 連接 →</Link>
        </div>
        <div className="main-ai-entry-actions">
          <Link href="/law?mode=advisor"><b aria-hidden="true">⌕</b><span><strong>考試顧問</strong><small>找教材・排讀書路線</small></span><em aria-hidden="true">→</em></Link>
          <Link href="/law?mode=coach"><b aria-hidden="true">✓</b><span><strong>陪考教練</strong><small>陪練・追蹤弱點與進度</small></span><em aria-hidden="true">→</em></Link>
        </div>
      </section>
...
export default function HomePage() {
  return <SimpleChatHome brand="高點知識達" logoSrc="/highpoint-logo.png" greeting="您的最佳考試顧問" />;
}
