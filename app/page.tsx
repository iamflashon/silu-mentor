...
function TeachingEvidenceDetails({ evidence }: { evidence?: TeachingEvidence | null }) {
  ...
  const label = isTeacherSolution ? "🟢 已鎖定本題老師解析／擬答" : evidence.status === "verified" ? "🟢 教材原文直接支持本次教學內容" : evidence.status === "applied_inference" ? "🔵 教材提供判準，AI 依原文涵攝" : evidence.status === "full_text_search" ? (evidence.retrieval === "full_text_search" ? "🟡 僅命中全文索引" : "🟡 已找到相關原文，直接支持不足") : "⚪ 未取得教材原文";
...
export default function Home() {
  ...
  function requestSolReview(message: Message, index: number) {
    const originalPrompt = pairedStudentPrompt(messages, index);
    void send(`你是 Sol 學霸，請獨立覆核 Luna 助教的回答。若本題有老師解析／擬答，必須以老師原文校準第一層標題、行為人順序、罪名順序與結論。逐項標示應保留、修正及補充之處；不同學說只能列為補充爭議，不得悄悄取代老師採說，也不得補造題目事實。\n\n【學生原問題】\n${originalPrompt}\n\n【Luna 助教回答】\n${cleanMessageText(message.text)}\n\n最後請依老師原本順序直接給出修正版。`, "sol");
  }
