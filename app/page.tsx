  ...
  return (
    <main className="coach-shell">
      ...
      <aside className="command-rail">
        ...
        <section className="rail-card rail-focus"><div className="rail-focus-meta"><span>距目標約 {dashboard?.monthsRemaining ?? "—"} 個月</span></div>{homeFeed?.course && <a href={homeFeed.course.sourceUrl} target="_blank" rel="noreferrer"><i>課</i><div><b>{homeFeed.course.title}</b><small>{homeFeed.recommended[0]?.summary || "依今日進度觀看重點片段"}</small></div></a>}{homeFeed?.book && <div className="rail-focus-book"><img src={`/api/resources/cover?id=${homeFeed.book.id}`} alt="推薦書封" /><div><b>{homeFeed.book.title}</b><small>{homeFeed.book.creator}</small></div></div>}{homeFeed?.magazine && <a href={homeFeed.magazine.sourceUrl} target="_blank" rel="noreferrer"><i>刊</i><div><b>{homeFeed.magazine.title}</b><small>本期試讀與文章</small></div></a>}<div className="rail-focus-law"><i>法</i><div><b>刑法第 271 條</b><small>殺人罪的基本構成要件</small></div></div></section>
