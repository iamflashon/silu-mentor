...
export default function NotesPage() {
  ...
  function chooseFilter(next: Filter) { setFilter(next); setPage(1); }
  ...
  return <main className="notes-page">
    <header className="notes-page-header"><a href="/" className="notes-home-link"><span>律</span>回首頁</a><div><p>MY STUDY NOTES</p><h1>我的筆記</h1><small>收藏與筆記集中在這裡，不必進入學習專區。</small></div><button type="button" onClick={() => setDraft(emptyDraft())}>＋ 新增筆記</button></header>
    <section className="notes-toolbar"><div className="notes-filters" role="group" aria-label="筆記類型"><button className={filter === "all" ? "active" : ""} onClick={() => chooseFilter("all")}>全部 <b>{notes.length}</b></button><button className={filter === "favorite" ? "active" : ""} onClick={() => chooseFilter("favorite")}>快速收藏 <b>{notes.filter((note) => note.sourceType === "favorite").length}</b></button><button className={filter === "note" ? "active" : ""} onClick={() => chooseFilter("note")}>我的整理 <b>{notes.filter((note) => note.sourceType !== "favorite").length}</b></button></div><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="搜尋標題、內容、科目、標籤或來源…" aria-label="搜尋我的筆記" /></section>
    {loading ? <div className="notes-empty">正在讀取我的筆記…</div> : visible.length ? <section className="standalone-note-list">{visible.map((note) => <article key={note.id} onClick={() => setDraft(note)}><div className="standalone-note-tags"><span>{note.sourceType === "favorite" ? "快速收藏" : "我的筆記"}</span><em>{note.subject}</em>{note.tags && <i>{note.tags}</i>}</div><h2>{note.title}</h2><p>{note.content}</p>{note.attachments?.[0] && <img src={note.attachments[0].url} alt="筆記附件" loading="lazy" />}{note.sourceLabel && <small>來源：{note.sourceLabel}</small>}<button type="button">查看／編輯</button></article>)}</section> : <div className="notes-empty"><b>{query ? "找不到符合條件的筆記" : "還沒有筆記"}</b><span>{query ? "可改用科目、爭點或來源名稱搜尋。" : "你可以在任何內容按「快速收藏」或「整理成筆記」，也能從這裡新增空白筆記。"}</span></div>}
