...
export default function NotesPage() {
  ...
  return <main className="notes-page">
    <header className="notes-page-header"><a href={isPengli ? "/teachers/pengli" : "/law"} className="notes-home-link"><span>{isPengli ? "狸" : "律"}</span>{isPengli ? "回彭狸專區" : "回首頁"}</a><div><p>MY STUDY NOTES</p><h1>{isPengli ? "彭狸老師・我的筆記" : "我的筆記"}</h1><small>{isPengli ? "只收錄彭狸老師專區的行政法收藏與整理。" : "收藏與筆記集中在這裡，不必進入學習專區。"}</small></div><button type="button" onClick={() => setDraft(emptyDraft())}>＋ 新增筆記</button></header>
