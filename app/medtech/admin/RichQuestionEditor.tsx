...
export function RichQuestionEditor({label,value,onChange,compact=false,category="medtech"}:{label:string;value:string;onChange:(value:string)=>void;compact?:boolean;category?:"medtech"|"accounting"}){
  ...
  function editTable(action:"rowAbove"|"rowBelow"|"deleteRow"|"colLeft"|"colRight"|"deleteCol"|"mergeRight"|"split"|"toggleBorders"){
  ...
  {selectedCell&&<div className="table-context-toolbar" onMouseDown={event=>event.preventDefault()}><span>表格編輯</span><button type="button" onClick={()=>editTable("toggleBorders")}>{selectedCell.closest("table")?.dataset.borders==="hidden"?"顯示框線":"隱藏框線"}</button><button type="button" onClick={()=>editTable("rowAbove")}>上方加列</button><button type="button" onClick={()=>editTable("rowBelow")}>下方加列</button><button type="button" onClick={()=>editTable("deleteRow")}>刪除列</button><button type="button" onClick={()=>editTable("colLeft")}>左側加欄</button><button type="button" onClick={()=>editTable("colRight")}>右側加欄</button><button type="button" onClick={()=>editTable("deleteCol")}>刪除欄</button><button type="button" disabled={!selectedCell.nextElementSibling} onClick={()=>editTable("mergeRight")}>向右合併</button><button type="button" disabled={selectedCell.colSpan<=1} onClick={()=>editTable("split")}>拆分</button></div>}
