...
export function RichQuestionEditor({label,value,onChange,compact=false,category="medtech"}:{label:string;value:string;onChange:(value:string)=>void;compact?:boolean;category?:"medtech"|"accounting"}){
  ...
  function resizeColumn(percent:number){const cell=selectedCell;const table=cell?.closest("table");if(!cell||!table)return;table.style.tableLayout="fixed";[...table.rows].forEach(row=>{const target=row.cells[cell.cellIndex];if(target)target.style.width=`${percent}%`});sync()}
