...
export function RichQuestionEditor({label,value,onChange,compact=false,category="medtech"}:{label:string;value:string;onChange:(value:string)=>void;compact?:boolean;category?:"medtech"|"accounting"|"data-structure"}){
  ...
  function setCellBorder(cell:HTMLTableCellElement,side:"top"|"bottom"|"left"|"right",visible:boolean){
    ...
    cell.style[property]=visible?"1px solid #517d78":"0";
    ...
    if(neighbor instanceof HTMLElement){const opposite=side==="top"?"borderBottom":side==="bottom"?"borderTop":side==="left"?"borderRight":"borderLeft";neighbor.style[opposite]=visible?"1px solid #517d78":"0"}
  }
  function editTable(action:"rowAbove"|"rowBelow"|"deleteRow"|"colLeft"|"colRight"|"deleteCol"|"mergeRight"|"split"|"toggleBorders"|"borderTop"|"borderBottom"|"borderLeft"|"borderRight"|"borderOuter"|"borderInner"|"borderAll"|"borderClear"|"alignLeft"|"alignCenter"|"alignRight"|"alignColumnLeft"|"alignColumnCenter"|"alignColumnRight"){
    ...
    if(action==="borderClear")targets.forEach(item=>(["top","bottom","left","right"] as const).forEach(side=>setCellBorder(item,side,false)));
