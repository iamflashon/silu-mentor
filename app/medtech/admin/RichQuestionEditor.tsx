...
export function RichQuestionEditor({label,value,onChange,compact=false,category="medtech"}:{label:string;value:string;onChange:(value:string)=>void;compact?:boolean;category?:"medtech"|"accounting"}){
  ...
  function clearSelection(){window.getSelection()?.removeAllRanges();selectionRef.current=null;setFormatState({bold:false,italic:false,underline:false,unorderedList:false})}
  ...
  function refreshFormatState(){try{setFormatState({bold:document.queryCommandState("bold"),italic:document.queryCommandState("italic"),underline:document.queryCommandState("underline"),unorderedList:document.queryCommandState("insertUnorderedList")})}catch{setFormatState({bold:false,italic:false,underline:false,unorderedList:false})}}
  ...
  <div ref={ref} className="rich-canvas" contentEditable suppressContentEditableWarning data-placeholder={`輸入${label}，也可以直接貼上 Word 內容或截圖`} onClick={event=>{rememberSelection();refreshFormatState();const cell=(event.target as HTMLElement).closest("td,th");setSelectedCell(cell&&ref.current?.contains(cell)?cell as HTMLTableCellElement:null)}} onFocus={()=>{rememberSelection();refreshFormatState()}} onSelect={()=>{rememberSelection();refreshFormatState()}} onKeyUp={()=>{rememberSelection();refreshFormatState()}} onMouseUp={()=>{rememberSelection();refreshFormatState()}} onInput={()=>{sync();refreshFormatState()}} onBlur={sync} onPaste={paste}/><small>可直接貼上 Word 格式與螢幕截圖；圖片會保存到醫檢題庫空間。</small></label>
}
