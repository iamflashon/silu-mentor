...
export function RichQuestionEditor({label,value,onChange,compact=false,category="medtech",highlightIssues=true,documentId}:{label:string;value:string;onChange:(value:string)=>void;compact?:boolean;category?:"medtech"|"accounting"|"data-structure";highlightIssues?:boolean;documentId?:number}){
  const ref=useRef<HTMLDivElement>(null); const fileRef=useRef<HTMLInputElement>(null); const selectionRef=useRef<Range|null>(null); const imageFiles=useRef(new Map<string,File>()); const svgHistory=useRef<string[]>([]); const svgFuture=useRef<string[]>([]); const [showSymbols,setShowSymbols]=useState(false); const [showTableGrid,setShowTableGrid]=useState(false); const [showBorderPicker,setShowBorderPicker]=useState(false); const [borderWidth,setBorderWidth]=useState<1|2|3>(1); const [borderGap,setBorderGap]=useState(24); const [gridSize,setGridSize]=useState({rows:3,cols:4}); const [selectedCell,setSelectedCell]=useState<HTMLTableCellElement|null>(null); const [selectedCells,setSelectedCells]=useState<HTMLTableCellElement[]>([]); const [selectedImage,setSelectedImage]=useState<HTMLImageElement|null>(null); const [selectedDiagram,setSelectedDiagram]=useState<SVGSVGElement|null>(null); const [selectedSvgItem,setSelectedSvgItem]=useState<Element|null>(null); const [svgItemText,setSvgItemText]=useState(""); const [diagramMode,setDiagramMode]=useState<"select"|"connect">("select"); const [connectionStart,setConnectionStart]=useState<SVGGElement|null>(null); const [newEdgeDirected,setNewEdgeDirected]=useState(false); const [uploading,setUploading]=useState(false); const [convertingTable,setConvertingTable]=useState(false); const [convertingDiagram,setConvertingDiagram]=useState(false); const [structureType,setStructureType]=useState(""); const [formatState,setFormatState]=useState({bold:false,italic:false,underline:false,unorderedList:false,alignment:"left" as "left"|"center"|"right"});
  ...
  async function upload(file:File){
    ...
    const form=new FormData();form.set("file",file);if(category==="medtech"&&documentId)form.set("documentId",String(documentId));const response=await fetch(category==="accounting"?"/api/accounting/admin/question-assets":`/api/medtech/admin/question-assets${documentId?`?documentId=${documentId}`:""}`,{method:"POST",body:form});const data=await response.json() as {url?:string;error?:string};
  ...
  function editTable(action:"rowAbove"|"rowBelow"|"deleteRow"|"colLeft"|"colRight"|"deleteCol"|"mergeRight"|"split"|"toggleBorders"|"borderTop"|"borderBottom"|"borderInsetBottom"|"borderLeft"|"borderRight"|"borderOuter"|"borderInner"|"borderAll"|"borderClear"|"alignLeft"|"alignCenter"|"alignRight"|"alignColumnLeft"|"alignColumnCenter"|"alignColumnRight"){
    ...
    if(action==="borderInsetBottom")targets.forEach(item=>{item.style.borderBottom="0";item.style.backgroundImage=`linear-gradient(#517d78,#517d78)`;item.style.backgroundRepeat="no-repeat";item.style.backgroundPosition="center bottom";item.style.backgroundSize=`calc(100% - ${borderGap}px) ${borderWidth}px`});
