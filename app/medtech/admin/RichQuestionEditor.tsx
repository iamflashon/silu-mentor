...
export function RichQuestionEditor({label,value,onChange,compact=false,category="medtech"}:{label:string;value:string;onChange:(value:string)=>void;compact?:boolean;category?:"medtech"|"accounting"|"data-structure"}){
  ...
  {selectedDiagram&&<div className="diagram-context-toolbar"><span>圖形大小</span><TableSizeControl key={`diagram-${selectedDiagram.closest<HTMLElement>("figure")?.dataset.displayPercent||"100"}`} label="圖形寬" min={30} max={100} value={Number(selectedDiagram.closest<HTMLElement>("figure")?.dataset.displayPercent)||100} onChange={resizeDiagram}/><small>可拖曳、輸入數字或用 ± 微調</small></div>}
