...
export function RichQuestionEditor({label,value,onChange,compact=false,category="medtech"}:{label:string;value:string;onChange:(value:string)=>void;compact?:boolean;category?:"medtech"|"accounting"|"data-structure"}){
  const ref=useRef<HTMLDivElement>(null); const fileRef=useRef<HTMLInputElement>(null); const selectionRef=useRef<Range|null>(null); const imageFiles=useRef(new Map<string,File>()); const [showSymbols,setShowSymbols]=useState(false); const [showTableGrid,setShowTableGrid]=useState(false); const [showBorderPicker,setShowBorderPicker]=useState(false); const [gridSize,setGridSize]=useState({rows:3,cols:4}); const [selectedCell,setSelectedCell]=useState<HTMLTableCellElement|null>(null); const [selectedImage,setSelectedImage]=useState<HTMLImageElement|null>(null); const [selectedDiagram,setSelectedDiagram]=useState<SVGSVGElement|null>(null); const [uploading,setUploading]=useState(false); const [convertingTable,setConvertingTable]=useState(false); const [convertingDiagram,setConvertingDiagram]=useState(false); const [structureType,setStructureType]=useState(""); const [formatState,setFormatState]=useState({bold:false,italic:false,underline:false,unorderedList:false,alignment:"left" as "left"|"center"|"right"});
  ...
  useEffect(()=>{const normalized=normalizeTemperature(value||"");if(ref.current&&ref.current.innerHTML!==normalized)ref.current.innerHTML=normalized;if(ref.current)cropStructureDiagrams(ref.current)},[value]);
  function sync(){const normalized=normalizeTemperature(ref.current?.innerHTML??"");if(ref.current&&ref.current.innerHTML!==normalized)ref.current.innerHTML=normalized;onChange(normalized);rememberSelection()}
  ...
  function editTable(action:"rowAbove"|"rowBelow"|"deleteRow"|"colLeft"|"colRight"|"deleteCol"|"mergeRight"|"split"|"toggleBorders"|"borderTop"|"borderBottom"|"borderLeft"|"borderRight"|"alignLeft"|"alignCenter"|"alignRight"|"alignColumnLeft"|"alignColumnCenter"|"alignColumnRight"){
    ...
    if(action==="borderTop"||action==="borderBottom"||action==="borderLeft"||action==="borderRight"){
      ...
      const currentlyHidden=cell.style[property]==="0px"||cell.style[property]==="0"||cell.style[property]==="none";
      cell.style[property]=currentlyHidden?"1px solid #a9c8c6":"0";
      const neighbor=side==="left"?cell.previousElementSibling:side==="right"?cell.nextElementSibling:table.rows[row.rowIndex+(side==="top"?-1:1)]?.cells[cellIndex];
      if(neighbor instanceof HTMLElement){const opposite=side==="top"?"borderBottom":side==="bottom"?"borderTop":side==="left"?"borderRight":"borderLeft";neighbor.style[opposite]=currentlyHidden?"1px solid #a9c8c6":"0"}
    }
  ...
  {selectedCell&&<div className="table-context-toolbar" onMouseDown={event=>{if((event.target as HTMLElement).closest("button"))event.preventDefault()}}><span>表格編輯</span><div className="cell-border-control"><button type="button" className={showBorderPicker?"active":""} onClick={()=>setShowBorderPicker(value=>!value)}>框線設定</button>{showBorderPicker&&<div className="cell-border-picker" aria-label="指定儲存格框線"><button type="button" className={`top ${selectedCell.style.borderTop==="0px"?"hidden":""}`} title="切換上框線" onClick={()=>editTable("borderTop")}>上</button><button type="button" className={`left ${selectedCell.style.borderLeft==="0px"?"hidden":""}`} title="切換左框線" onClick={()=>editTable("borderLeft")}>左</button><span>目前格</span><button type="button" className={`right ${selectedCell.style.borderRight==="0px"?"hidden":""}`} title="切換右框線" onClick={()=>editTable("borderRight")}>右</button><button type="button" className={`bottom ${selectedCell.style.borderBottom==="0px"?"hidden":""}`} title="切換下框線" onClick={()=>editTable("borderBottom")}>下</button><small>深色＝顯示；淡色虛線＝隱藏</small></div>}</div><div className="table-align-group"><b>本格</b><button type="button" onClick={()=>editTable("alignLeft")}>左</button><button type="button" onClick={()=>editTable("alignCenter")}>中</button><button type="button" onClick={()=>editTable("alignRight")}>右</button></div><div className="table-align-group"><b>整欄</b><button type="button" onClick={()=>editTable("alignColumnLeft")}>左</button><button type="button" onClick={()=>editTable("alignColumnCenter")}>中</button><button type="button" onClick={()=>editTable("alignColumnRight")}>右</button></div><TableSizeControl key={`table-${selectedCell.closest("table")?.style.width||"100"}`} label="表格寬" min={40} max={100} value={parseInt(selectedCell.closest("table")?.style.width||"100",10)||100} onChange={resizeTable}/><TableSizeControl key={`column-${logicalColumnIndex(selectedCell)}-${currentColumnWidth(selectedCell)}`} label="本欄寬" min={10} max={70} value={currentColumnWidth(selectedCell)} onChange={resizeColumn}/><button type="button" onClick={()=>editTable("toggleBorders")}>{selectedCell.closest("table")?.dataset.borders==="hidden"?"顯示全部":"隱藏全部"}</button><button type="button" onClick={()=>editTable("rowAbove")}>上方加列</button><button type="button" onClick={()=>editTable("rowBelow")}>下方加列</button><button type="button" onClick={()=>editTable("deleteRow")}>刪除列</button><button type="button" onClick={()=>editTable("colLeft")}>左側加欄</button><button type="button" onClick={()=>editTable("colRight")}>右側加欄</button><button type="button" onClick={()=>editTable("deleteCol")}>刪除欄</button><button type="button" disabled={!selectedCell.nextElementSibling} onClick={()=>editTable("mergeRight")}>向右合併</button><button type="button" disabled={selectedCell.colSpan<=1} onClick={()=>editTable("split")}>拆分</button></div>}
  {selectedDiagram&&<div className="diagram-context-toolbar" onMouseDown={event=>{if((event.target as HTMLElement).closest("button,input"))event.preventDefault()}}><span>圖形大小</span><TableSizeControl key={`diagram-${selectedDiagram.closest<HTMLElement>("figure")?.dataset.displayPercent||"100"}`} label="圖形寬" min={30} max={100} value={Number(selectedDiagram.closest<HTMLElement>("figure")?.dataset.displayPercent)||100} onChange={resizeDiagram}/><small>可拖曳、輸入數字或用 ± 微調</small></div>}
  <div ref={ref} className="rich-canvas" contentEditable suppressContentEditableWarning data-placeholder={`輸入${label}，也可以直接貼上 Word 內容或截圖`} onClick={event=>{rememberSelection();refreshFormatState();const target=event.target as HTMLElement;const cell=target.closest("td,th");setSelectedCell(cell&&ref.current?.contains(cell)?cell as HTMLTableCellElement:null);const image=target.closest("img");setSelectedImage(image&&ref.current?.contains(image)?image as HTMLImageElement:null);const diagram=target.closest("svg");setSelectedDiagram(diagram&&ref.current?.contains(diagram)?diagram as SVGSVGElement:null)}} onFocus={()=>{rememberSelection();refreshFormatState()}} onSelect={()=>{rememberSelection();refreshFormatState()}} onKeyUp={()=>{rememberSelection();refreshFormatState()}} onMouseUp={()=>{rememberSelection();refreshFormatState()}} onInput={()=>{sync();refreshFormatState()}} onBlur={sync} onPaste={paste}/><small>可直接貼上 Word 格式與螢幕截圖；點選圖片後可按「圖片轉表格」；點選 SVG 圖形可調整顯示大小。</small></div>
}
