...
export function RichQuestionEditor({label,value,onChange,compact=false}:{label:string;value:string;onChange:(value:string)=>void;compact?:boolean}){
  ...
  return <label className={`rich-field ${compact?"compact":""}`}><span>{label}</span><div className="rich-toolbar" role="toolbar" aria-label={`${label}格式工具`}>
    ...
    <button type="button" title="上標" onClick={()=>command("superscript")}>x²</button><button type="button" title="下標" onClick={()=>command("subscript")}>x₂</button><button type="button" title="項目符號" onClick={()=>command("insertUnorderedList")}>• 清單</button>
