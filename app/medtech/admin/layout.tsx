...
export default async function MedtechAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return <MedtechAdminAccessProvider
    fullAdmin={auth.access.canAdmin}
    questionEditor={auth.access.canAdmin || hasMedtechPermission(auth.access.permissionsJson, "questions") || hasMedtechPermission(auth.access.permissionsJson, "document-library")}
    documentLibraryEditor={!auth.access.canAdmin && hasMedtechPermission(auth.access.permissionsJson, "document-library")}
  >{children}</MedtechAdminAccessProvider>;
}
