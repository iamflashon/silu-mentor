...
type Question = {
  ...
  explanation: string; teacherAnswer?: string; aiCompleteExplanation?: string; teacherCompleteExplanation?: string; completeExplanation?: string; answerSource: string; status: string;
...
export default function QuestionWorkbenchPage({ category = "medtech" }: { category?: Category }) {
  ...
  return <main className="standalone-workbench">
    ...
    <section className="standalone-workbench-grid"><SourceWorkspace/><article className="standalone-question-editor">
      <div className="question-editor-meta"><label>年份<input value={item.year} onChange={event => setItem({ ...item, year: event.target.value })}/></label><label>科目<input value={item.subject} onChange={event => setItem({ ...item, subject: event.target.value })}/></label><label>題號<input value={item.questionNumber} onChange={event => setItem({ ...item, questionNumber: event.target.value })}/></label>{!essay && <label>{item.isSimulation ? "老師答案" : "正式答案"}<select value={item.teacherAnswer || item.correctAnswer || ""} onChange={event => setItem({ ...item, teacherAnswer: event.target.value, correctAnswer: event.target.value || null })}><option value="">未設定</option>{["A", "B", "C", "D"].map(letter => <option key={letter}>{letter}</option>)}</select></label>}</div>
      ...
      {!essay && <section className="simulation-workbench-fields explanation-version-fields"><h2>完整解析版本</h2><p>AI 版與老師版分開保存；老師版未填前，前台不會冒充老師解析。</p><RichQuestionEditor label="AI 完整解析（AI 版／待老師核對）" value={item.aiCompleteExplanation || (item.isSimulation ? (item.simulatedCompleteExplanation || "") : "")} onChange={value => setItem(item.isSimulation ? { ...item, simulatedCompleteExplanation: value, aiCompleteExplanation: value } : { ...item, aiCompleteExplanation: value })}/><RichQuestionEditor label="老師完整解析（老師版／語音文本）" value={item.teacherCompleteExplanation || ""} onChange={value => setItem({ ...item, teacherCompleteExplanation: value })}/></section>}
