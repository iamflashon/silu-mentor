...
type ChallengeResult = { score: number; total: number; averageSeconds: number; reward: Reward; attemptsRemaining: number };
...
export default function MedtechPackDiscount({ packageName, packNumber, questionTotal, label, href }: Props) {
  ...
  const [challengeAttemptsRemaining, setChallengeAttemptsRemaining] = useState(2);
  ...
  function chooseChallengeAnswer(answer: string) {
    ...
    const seconds = Math.max(0, Math.min(5, (Date.now() - challengeStartedAtRef.current) / 1000));
  ...
  async function submitChallenge() {
    ...
    try {
      const response = await fetch("/api/medtech/question-pack-reward", {
        ...
        body: JSON.stringify({ packageName, pack: packNumber, action: "quiz", questionIds: challengeQuestions.map((question) => question.id), answers: Object.entries(challengeAnswersRef.current).map(([questionId, answer]) => ({ questionId: Number(questionId), answer })), timings: Object.entries(challengeTimingsRef.current).map(([questionId, seconds]) => ({ questionId: Number(questionId), seconds })) }),
      });
      const result = await response.json() as { score?: number; total?: number; averageSeconds?: number; attemptsRemaining?: number; reward?: Reward; error?: string };
      if (!response.ok || !result.reward) throw new Error(result.error || "答題挑戰暫時無法完成，請稍後再試。");
      const nextResult = { score: result.score ?? 0, total: result.total ?? challengeQuestions.length, averageSeconds: result.averageSeconds ?? 5, attemptsRemaining: result.attemptsRemaining ?? 0, reward: result.reward };
  ...
  function advanceChallenge(force = false) {
    ...
    if (!challengeTimingsRef.current[question.id]) challengeTimingsRef.current[question.id] = 5;
  ...
  const canChallengeAgain = Boolean(reward && (reward.quizAttemptsRemaining ?? 0) > 0 && reward.percent !== 50 && (isOriginal || (reward.quizAttemptsUsed ?? 0) > 0));
