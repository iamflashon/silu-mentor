...
const rules = [
  ["章節／隨機題目包", "30 點／包", "任選一包 30 題免費體驗一次；之後每一關可抽一次轉轉樂，最高五折，放棄後以 30 點解鎖；開通後 7 天內不限次數重做。"],
...
const usageSteps = [
  ["先領取題目包", "任選一包免費／轉轉樂", "每包 30 題；免費包使用一次後，下一關可抽一次折扣，抽完或放棄後再用點數解鎖，頁面會顯示到期倒數。"],
...
export default function MedtechPricingPage() {
  return <main className="medtech-pricing-page">
    ...
    <section className="medtech-pricing-head">
      <span>簡單點數制</span>
      <h1>不用訂閱，按照使用方式扣點。</h1>
      <p>1 點＝NT$1。首次登入贈送 10 點；任選一包免費初體驗。完成上一關，下一關還能抽一次限時轉轉樂，最高五折，想再次刷題再購買題目包。</p>
    </section>
