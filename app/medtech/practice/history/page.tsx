...
export default async function MedtechPracticeHistory({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  return <main className="medtech-practice medtech-history-page">
    ...
    <section className="medtech-history-shell">
      ...
      {!sessions.length ? <section className="medtech-history-empty"><b>目前還沒有這個題目包的作答紀錄。</b><span>完成一回練習後，這裡會留下開始時間、完成狀態與逐題檢討。</span></section> : <div className="medtech-history-list">
        {sessions.map((session) => {
          ...
          return <article className={`medtech-history-card ${session.completedAt ? "completed" : "unfinished"}`} key={session.id}>
            <header><div><span>{session.packageName} · 第 {session.packNumber} 關</span><h2>{statusLabel(session.status, session.completedAt)}</h2><small>開始：{formatDate(session.startedAt)} · 最後活動：{formatDate(session.lastActiveAt)}</small></div><div className="medtech-history-card-actions">{!session.completedAt && session.status !== "expired" && <a className="primary" href={`/medtech/practice?${session.packageName === "隨機模考" ? `pack=${session.packNumber}` : `topic=${encodeURIComponent(session.packageName)}&pack=${session.packNumber}`}`}>繼續作答</a>}<a href={`/medtech/practice/history?${session.packageName === "隨機模考" ? `pack=${session.packNumber}` : `topic=${encodeURIComponent(session.packageName)}&pack=${session.packNumber}`}`}>重新整理</a></div></header>
            ...
            {session.completedAt && <div className="medtech-history-insights"><span>最久思考：{longest ? `第 ${longest.order + 1} 題 · ${formatDuration(longest.durationSeconds)}` : "尚無逐題時間"}{longestWrong ? " · 思考最久且答錯，請特別複習" : ""}</span><span>重複答錯：{repeatedWrong.length ? `${repeatedWrong.length} 題` : "無"}</span>{weaknesses.slice(0, 3).map((item) => <span key={item.label}>需加強：{item.label}（{item.count} 題）</span>)}</div>}
