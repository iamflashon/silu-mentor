...
export default function MedtechPractice() {
  ...
  async function choosePackDiscount(action: "spin" | "abandon") {
    if (discountAction || !packageAccess?.locked || packageAccess.blockedByPrevious) return;
    ...
    try {
      const response = await fetch("/api/medtech/question-pack-reward", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ packageName: topic || "隨機模考", pack, action }),
      });
      ...
      setPackageAccess((current) => current ? { ...current, discountReward: result.reward, cost: result.reward?.cost ?? current.cost } : current);
    } catch (reason) {
    ...
    <main className="medtech-practice">
      ...
      {!wrongOnly && packageAccess && <section className={`medtech-package-status ${packageAccess.locked ? "locked" : "active"}`}>
        <div>
          ...
          <span>{packageAccess.blockedByPrevious ? "完成上一關後，下一關會自動開放。" : packageAccess.locked ? `原價 NT$${packageAccess.baseCost ?? 30}；購買後 ${packageAccess.days} 天內不限次數重做。` : packageRemaining !== null ? `剩餘 ${formatRemaining(packageRemaining)}；請把握時間完成練習。` : `${packageAccess.days} 天內不限次數重做。`}</span>
        ...
        {packageAccess.locked && !packageAccess.blockedByPrevious && packageAccess.discountReward && packageAccess.discountReward.status !== "available" && <button type="button" onClick={() => void unlockPackage()} disabled={unlockingPackage} aria-busy={unlockingPackage}>{unlockingPackage ? "處理中…" : `NT$${packageAccess.cost} 購買`}</button>}
        {packageAccess.locked && !packageAccess.blockedByPrevious && packageAccess.discountReward?.status === "available" && <div className="medtech-pack-discount-box"><div><b>🎡 轉轉樂｜這一關只有一次機會</b><span>最高五折；完成前一關後另可挑戰隨機 10 題，每題 5 秒，每包最多 2 次；放棄轉轉樂後就回到原價 30 點。</span></div><div><button type="button" onClick={() => void choosePackDiscount("spin")} disabled={Boolean(discountAction)} aria-busy={discountAction === "spin"}>{discountAction === "spin" ? <span className="medtech-loading-label"><i className="medtech-loading-spinner" aria-hidden="true"/>抽取中…</span> : "抽一次折扣"}</button><button type="button" className="secondary" onClick={() => void choosePackDiscount("abandon")} disabled={Boolean(discountAction)}>{discountAction === "abandon" ? "處理中…" : "放棄，原價購買"}</button></div></div>}
        {packageAccess.locked && !packageAccess.blockedByPrevious && packageAccess.discountReward?.status === "revealed" && <div className="medtech-pack-discount-box revealed"><div><b>🎉 抽到{packageAccess.discountReward.label}｜優惠價 NT${"$"}{packageAccess.discountReward.cost}</b><span>這個折扣只適用本關一次；購買後 7 天內不限次數重做。</span></div></div>}
        {packageAccess.locked && !packageAccess.blockedByPrevious && packageAccess.discountReward?.status === "abandoned" && <div className="medtech-pack-discount-box abandoned"><div><b>已放棄本關折扣｜原價 NT$30</b><span>轉轉樂每一關只有一次機會；現在可以直接用原價購買。</span></div></div>}
      </section>}
      ...
      <div className="medtech-exam-grid">
        ...
        <section className="medtech-question">
          ...
          <footer>
            ...
            <span>{q.locked ? `本題屬於題目包；售價 NT$${packageAccess?.cost ?? 30}` : "答案在完成前不顯示"}</span>
      ...
      {paywallOpen && <div className="medtech-paywall-backdrop" role="presentation" onMouseDown={() => setPaywallOpen(false)}><section className="medtech-paywall" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><span>醫檢師題目包</span><h2>購買第 {packageAccess?.packageNumber ?? pack} 關</h2><p>這一關共 {packageAccess?.questionCount ?? 30} 題。購買後 7 天內不限次數重做，並會保存完成時間、答題時間、錯題與需要加強的觀念。</p>{packageAccess?.discountReward?.status === "available" && <div className="medtech-paywall-wheel"><b>🎡 這一關可抽一次折扣，最高五折</b><span>完成前一關後，另可挑戰隨機 10 題，每題 5 秒，每包最多 2 次；抽完或放棄後，再以優惠價或原價購買。</span><div><button type="button" onClick={() => void choosePackDiscount("spin")} disabled={Boolean(discountAction)}>{discountAction === "spin" ? "抽取中…" : "抽一次折扣"}</button><button type="button" className="secondary" onClick={() => void choosePackDiscount("abandon")} disabled={Boolean(discountAction)}>{discountAction === "abandon" ? "處理中…" : "放棄，原價購買"}</button></div></div>}{packageAccess?.discountReward?.status === "revealed" && <p className="medtech-paywall-result">已抽到{packageAccess.discountReward.label}：NT${"$"}{packageAccess.discountReward.cost}。</p>}{packageAccess?.discountReward && (packageAccess.discountReward.status === "revealed" || packageAccess.discountReward.status === "abandoned") && <button type="button" className="medtech-paywall-credit" onClick={() => void unlockPackage()} disabled={unlockingPackage}>{unlockingPackage ? "處理中…" : `NT$${packageAccess.cost} 購買`}</button>}<div><button type="button" onClick={() => setPaywallOpen(false)}>稍後再說</button><Link href="/medtech/upgrade?reason=question-pack">查看購買方式</Link></div></section></div>}
    </main>
  );
}
