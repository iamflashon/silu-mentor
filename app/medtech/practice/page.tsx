...
export default function MedtechPractice() {
  ...
  return (
    <main className="medtech-practice">
      ...
      {paywallOpen && <div className="medtech-paywall-backdrop" role="presentation" onMouseDown={() => setPaywallOpen(false)}><section className="medtech-paywall" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><span>醫檢師題目包</span><h2>解鎖第 {packageAccess?.packageNumber ?? pack} 關</h2><p>這一關共 {packageAccess?.questionCount ?? 30} 題。開通後 7 天內不限次數重做，並會保存完成時間、答題時間、錯題與需要加強的觀念。</p>{packageAccess?.discountReward?.status === "available" && <div className="medtech-paywall-wheel"><b>🎡 這一關可抽一次折扣，最高五折</b><span>完成前一關後，另可挑戰隨機 10 題，每題 5 秒，每包最多 2 次；抽完或放棄後，才能決定用優惠價或原價解鎖。</span><div><button type="button" onClick={() => void choosePackDiscount("spin")} disabled={Boolean(discountAction)}>{discountAction === "spin" ? "抽取中…" : "抽一次折扣"}</button><button type="button" className="secondary" onClick={() => void choosePackDiscount("abandon")} disabled={Boolean(discountAction)}>{discountAction === "abandon" ? "處理中…" : "放棄，原價購買"}</button></div></div>}{packageAccess?.discountReward?.status === "revealed" && <p className="medtech-paywall-result">已抽到{packageAccess.discountReward.label}：{packageAccess.discountReward.cost} 點。</p>}{packageAccess?.discountReward && (packageAccess.discountReward.status === "revealed" || packageAccess.discountReward.status === "abandoned") && <button type="button" className="medtech-paywall-credit" onClick={() => void unlockPackage()} disabled={unlockingPackage}>{unlockingPackage ? "解鎖中…" : `用 ${packageAccess.cost} 點解鎖`}</button>}<div><button type="button" onClick={() => setPaywallOpen(false)}>稍後再說</button><Link href="/medtech/upgrade?reason=question-pack">前往購買點數</Link></div></section></div>}
    </main>
  );
}
