...
async function readResponseJson<T extends Record<string, unknown>>(
  response: Response,
) {
...
export default function MedtechAiStudy() {
  ...
  function appendGuidedEvent(
    event: Omit<GuidedEvent, "at">,
    current = eventsRef.current,
  ) {
    const next = [...current, { ...event, at: new Date().toISOString() }].slice(
      -100,
    );
  ...
  function queueGuidedRecord(overrides: GuidedStateOverrides = {}) {
    ...
    const state = {
      ...
      correct:
        (overrides.selectedAnswer ?? selectedAnswer)
          ? (overrides.selectedAnswer ?? selectedAnswer) ===
      ...
      startedAt:
        overrides.startedAt ?? new Date(startedAtRef.current).toISOString(),
    ...
    recordSaveQueueRef.current = recordSaveQueueRef.current
      .catch(() => undefined)
      .then(async () => {
        const response = await fetch("/api/guided-practice", {
          method: "PUT",
  ...
  async function loadQuestion(nextTopic = topic) {
    ...
    try {
      ...
      if (!response.ok || !result.items?.[0])
        throw new Error(result.error || "目前沒有可用題目");
      ...
      eventsRef.current = [
        {
          ...
          label: "抽到題目",
          detail: `${nextQuestion.year} 年第 ${nextQuestion.questionNumber || nextQuestion.id} 題`,
  ...
  async function requestCachedTutor(mode: "hint" | "compare") {
    if (
      !question ||
      loading ||
      (mode === "hint" && (hintUsed || selectedAnswer)) ||
      (mode === "compare" && (!selectedAnswer || comparisonUsed))
    )
      return;
    const prompt =
      mode === "hint"
    ...
    try {
      const response = await fetch("/api/medtech/tutor", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
      ...
      if (!response.ok || !result.reply)
        throw new Error(result.error || "AI 回答失敗");
      if (mode === "compare") setComparisonUsed(true);
      const mentorMessage: Message = {
        role: "mentor",
        text: plainText(result.reply!),
        source: result.source,
        usage: result.usage,
      };
      const finalMessages = [...next, mentorMessage];
      const nextEvents = appendGuidedEvent({
        type: mode === "hint" ? "hint" : "compare",
        label: mode === "hint" ? "取得判斷提示" : "完成比較選項",
      });
      setMessages(finalMessages);
      void queueGuidedRecord({
        messages: finalMessages,
        events: nextEvents,
        hintUsed: mode === "hint" ? true : hintUsed,
        comparisonUsed: mode === "compare" ? true : comparisonUsed,
      });
    } catch (caught) {
  ...
  const loadingLabel = (label: string) => (
    ...
    <main className="medtech-ai-page">
      ...
      <section className="medtech-ai-shell">
        <aside className="medtech-ai-settings">
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) => {
                ...
                void loadQuestion(event.target.value);
              }}
            >
          ...
          <button
            className="medtech-new-question"
            onClick={() => void loadQuestion()}
            disabled={questionLoading}
            aria-busy={questionLoading}
          >
            {questionLoading ? loadingLabel("題目載入中…") : "換一道題"}
          </button>
          <small>解題提示與選項比較皆為預先整理內容；即時 AI 追問目前暫停開放。</small>
          <div className="medtech-ai-record-link">
            <a href="/medtech/ai-study/history">查看引導學習紀錄 →</a>
            <span>
              {recordSaved ? "本題過程已保存" : "作答後自動保存完整過程"}
            </span>
          </div>
        </aside>
        <section className="medtech-ai-workspace">
          {questionLoading ? (
            <div className="medtech-ai-empty">
              {loadingLabel("正在從正式題庫抽題…")}
            </div>
          ) : question ? (
            <>
              <article className="medtech-ai-question">
                <header>
                  <span>{question.topic || topic || "臨床病毒學"}</span>
                  <small>{question.year} 年專技</small>
                </header>
                <h2>{question.stem}</h2>
                <div>
                  {Object.entries(question.options).map(([key, value]) => (
                    <button
                      type="button"
                      disabled={loading || Boolean(selectedAnswer)}
              ...
              <div className="medtech-ai-quick">
                <button
                  ...
                  aria-busy={loading}
                  onClick={() => void requestCachedTutor("hint")}
                >
                  {loading
                    ? loadingLabel("整理中…")
                    : hintUsed
                ...
                <button
                  ...
                  aria-busy={loading}
                  onClick={() => void requestCachedTutor("compare")}
                >
                  {loading
                    ? loadingLabel("比較中…")
                    : comparisonUsed
              ...
              <section className="medtech-ai-chat" ref={chatRef}>
                {messages.map((message, index) => (
                  <article className={message.role} key={index}>
                    <b>{message.role === "mentor" ? "解題引導" : "我"}</b>
                    <p>{plainText(message.text)}</p>
                    {message.role === "mentor" && message.source && (
                      <small>
                        {message.source}
                      </small>
                    )}
              ...
              <div className="medtech-live-ai-paused">
                即時 AI 追問目前暫停開放；本頁只提供已整理完成的解題內容。
              </div>
            </>
          ) : (
            <div className="medtech-ai-empty">
          ...
          {error && question && <p className="medtech-ai-error">{error}</p>}
        </section>
      </section>
      {paywall && (
        <div
        ...
        >
          ...
          >
            ...
            <p>{paywall.text}</p>
            <div>
              <button type="button" onClick={() => setPaywall(null)}>
                稍後再說
              </button>
              <a href={paywall.url}>選擇題目包</a>
            </div>
          </section>
        </div>
      )}
