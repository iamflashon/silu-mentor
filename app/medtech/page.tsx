...
export default async function MedtechHome() {
  ...
  return <main className="medtech-home">
    ...
    <section className="medtech-hero"><div><span>不只刷題｜把題目真正學會</span><h1>不只刷題，而是讓老師帶你真正把題目學會</h1><p className="medtech-hero-description">全真模擬試題 × 康情老師逐題語音解析 × AI 引導學習</p><p className="medtech-hero-offer">首次登入贈送 10 點｜任選一包 30 題免費｜開通後 7 天不限次數重做｜每關一次轉轉樂最高五折</p><p className="medtech-hero-suboffer">不用訂閱，依照使用方式簡單扣點；免費包用過一次，完成上一關後，下一關可抽一次限時轉轉樂，放棄或抽完後再用點數解鎖。</p><div className="medtech-hero-actions" data-no-navigation-feedback><MedtechPracticeEntry/><a href="/medtech/ai-study">進入引導學習</a><a href="/medtech/pricing">查看點數方式</a></div></div><aside><small>平台核心</small><b>名師 <em>×</em> AI</b><span>讓每一次作答都更接近學會</span><dl><div><dt>免費入口</dt><dd>任選一包</dd></div><div><dt>闖關誘因</dt><dd>轉轉樂最高五折</dd></div><div><dt>深度解析</dt><dd>老師語音</dd></div><div><dt>持續進步</dt><dd>學習紀錄</dd></div></dl></aside></section>
    ...
    <section className="medtech-home-points"><div><span>題目包制｜先體驗再決定</span><h2>便宜刷題，想再刷再購買</h2><p>首次登入贈 10 點；任選一包 30 題免費，使用一次後其他題目包以 30 點開通，7 天內不限次數重做。完成上一關，下一關就有一次限時轉轉樂，最高五折；抽完或放棄後再用點數解鎖。系統會提醒到期日，並保存刷題時間、答對率、錯題與需要加強的觀念；語音解析 1 點／24 小時，AI 追問 1 點。</p></div><a href="/medtech/pricing">查看題目包方式 →</a></section>
