...
export default async function MedtechAccountPage({ searchParams }: AccountPageProps) {
  ...
  return <main className="medtech-member-page"><header><a href="/medtech" className="medtech-brand"><span>醫</span><div><b>醫檢師備考</b><small>MY ACCOUNT</small></div></a></header><section className="medtech-member-card"><div className="identity"><span>{member.displayName.slice(0,1)}</span><div><small>醫檢師會員</small><h1>{member.displayName}</h1><p>{member.email}</p></div></div><div className="actions medtech-account-actions"><a className="primary" href="/medtech">進入學習首頁</a><a href="/medtech/upgrade">購買點數（測試付款）</a>{access.canAdmin && <a href="/medtech/admin">醫檢師管理後台</a>}<a href={chatGPTSignOutPath("/medtech")}>登出</a></div><dl><div><dt>類科資格</dt><dd>醫檢師 · 已開通</dd></div><div><dt>會員身分</dt><dd>{member.role === "teacher" ? "老師／導師" : "學員"}</dd></div><div><dt>班級</dt><dd>{access.className || "未分班"}</dd></div><div><dt>管理權限</dt><dd>{access.canAdmin ? "醫檢師管理員" : "一般會員"}</dd></div></dl><section className="medtech-point-summary"><div><small>目前可用點數</small><strong>{usage.aiCredits}<em> 點</em></strong><span>首次登入贈送 10 點</span></div><div><small>已使用點數</small><strong>{spent}<em> 點</em></strong><span>完整解析、語音與 AI 追問</span></div></section><section className="medtech-point-history"><header><div><small>點數明細</small><h2>使用狀況記錄</h2></div><span>第 {page}／{totalPages} 頁 · 共 {total} 筆</span></header>{history.length ? <div className="medtech-point-history-list">{history.map((row) => <article key={row.id}><div><b>{row.description}</b><small>{row.createdAt.toLocaleString("zh-TW", { dateStyle: "short", timeStyle: "short" })}</small></div><strong className={row.delta > 0 ? "plus" : "minus"}>{row.delta > 0 ? "+" : ""}{row.delta} 點</strong><span>餘額 {row.balanceAfter} 點</span></article>)}</div> : <p>目前還沒有點數使用紀錄。</p>}<nav className="medtech-point-pagination" aria-label="點數紀錄分頁"><a className={page <= 1 ? "disabled" : ""} href={page > 1 ? pageHref(page - 1) : pageHref(1)} aria-disabled={page <= 1}>上一頁</a><span>第 {page}／{totalPages} 頁</span><a className={page >= totalPages ? "disabled" : ""} href={page < totalPages ? pageHref(page + 1) : pageHref(totalPages)} aria-disabled={page >= totalPages}>下一頁</a></nav></section></section></main>;
}
