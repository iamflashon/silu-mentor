...
export default function ReviewPage() {
  ...
  async function startDebate() {
    ...
    try {
      const response = await fetch("/api/review", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ questionId: question.id, teacherModel, scholarModel: scholarModels[0], scholarModels, participantMode, stage: participantMode === "student-scholar" ? "start" : "full" }) });
  ...
  async function advanceAiStage(current: ReviewResult) {
    ...
    try {
      const response = await fetch("/api/review", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
        questionId: current.question.id, teacherModel, scholarModel: scholarModels[0], scholarModels, participantMode: "ai-scholar", stage: "next-stage", argumentStage: targetStage,
  ...
  function toggleScholarModel(model: Provider) {
    setScholarModels((current) => current.includes(model)
      ? current.length === 1 ? current : current.filter((item) => item !== model)
  ...
  return <main className="review-shell">
    ...
    <section className="review-workspace" id="battle">
      ...
      <div className="review-main"><div className="review-main-head"><div><p>{questionMeta}</p><h2>{activePanel === "question" ? "先讀題目，再安排對話" : activePanel === "debate" ? "老師問・學霸答" : "固定點評人的判斷"}</h2></div><span className="review-confidential">資料依據 <b>{question?.hasTeacherAnswer ? "已連結老師擬答" : "題目資料已連結"}</b></span></div>
        ...
        {activePanel === "question" && <section className="review-model-stage" id="model-pk"><div className="review-section-heading"><div><span>BEFORE THE DIALOGUE</span><h3>先安排老師與學霸</h3></div><p>可選 AI 學霸觀戰，也可以由你親自扮演學霸。固定點評人仍由 GPT-5.6 Sol 擔任，負責最後判斷與整理。</p></div><div className="review-participant-mode" aria-label="選擇學霸角色"><button type="button" className={participantMode === "ai-scholar" ? "selected" : ""} onClick={() => setParticipantMode("ai-scholar")}><span>AI</span><div><b>AI 學霸觀戰</b><small>由模型回答，適合比較不同模型</small></div></button><button type="button" className={participantMode === "student-scholar" ? "selected" : ""} onClick={() => setParticipantMode("student-scholar")}><span>我</span><div><b>同學扮演學霸</b><small>自己回答、不限時間，真正練習三段論法</small></div></button></div><div className="review-model-pickers"><ModelPicker tone="positive" title="老師／追問模型" subtitle="提出問題／抓出論證漏洞" value={teacherModel} onChange={setTeacherModel} />{participantMode === "ai-scholar" ? <><div className="review-versus">↔</div><ModelBattlePicker values={scholarModels} onToggle={toggleScholarModel} /></> : <div className="review-student-role"><span>我</span><b>你就是學霸</b><small>下方會出現輸入框，讀完老師問題後再自行作答。</small></div>}</div><div className="review-fixed-judge"><span className="review-judge-badge">S</span><div><b>固定 AI 點評人｜GPT-5.6 Sol</b><small>最後整理本回合真正爭點、誰抓到漏洞、哪裡仍需修正，以及考場應如何寫。</small></div><i>LOCKED</i></div>{participantMode === "ai-scholar" ? <div className="review-watch-settings"><div><b>對話方式</b><small>每次只出現一個小問題；多模型會讀取同一題、同一提示詞，再以表格並列回答。</small></div><div className="review-watch-mode"><button type="button" className={debateMode === "manual" ? "selected" : ""} onClick={() => setDebateMode("manual")}>手動繼續</button><button type="button" className={debateMode === "countdown" ? "selected" : ""} onClick={() => setDebateMode("countdown")}>倒數自動</button></div>{debateMode === "countdown" && <label className="review-countdown-select">每句間隔<select value={countdownSeconds} onChange={(event) => setCountdownSeconds(Number(event.target.value))}><option value={15}>15 秒</option><option value={20}>20 秒</option><option value={30}>30 秒</option><option value={45}>45 秒</option></select></label>}</div> : <div className="review-unlimited-note"><b>不限時間作答</b><span>沒有倒數、沒有自動送出。你可以先整理大前提，再寫小前提涵攝，完成後按送出。</span></div>}<button className="review-start-button" onClick={() => void startDebate()} disabled={!question || running}>{running ? <><span className="review-spinner" />正在準備老師的第一個問題…</> : <>{participantMode === "student-scholar" ? "開始，我來扮演學霸" : "開始老師與學霸的對話"} <span>→</span></>}</button></section>}
        {activePanel === "question" && <ReviewHistoryPanel history={history} onRestart={restartAttempt} />}
        {activePanel !== "question" && <DebatePanel key={`${question?.id ?? "none"}-${result?.argumentStage ?? "major-premise"}-${result?.teacherQuestion?.text ?? "loading"}`} question={question} result={result} running={running} mode={debateMode} countdownSeconds={countdownSeconds} participantMode={participantMode} teacherModel={teacherModel} completedRounds={completedRounds} onNextStage={(current) => void advanceAiStage(current)} onGoQuestion={() => setActivePanel("question")} onRestart={restartAttempt} onComplete={handleReviewComplete} />}
...
function ModelBattlePicker({ values, onToggle }: { values: Provider[]; onToggle: (model: Provider) => void }) {
  return <div className="review-model-battle-picker"><div className="review-picker-top"><span>霸</span><b>學霸模型對戰組合</b></div><p>選 1–3 個模型；每個模型會回答同一個老師問題，結果以表格並列。</p><div className="review-model-choice-list">{Object.entries(modelLabels).map(([key, label]) => { const model = key as Provider; return <button type="button" key={model} className={values.includes(model) ? "selected" : ""} onClick={() => onToggle(model)}><i>{values.includes(model) ? "✓" : ""}</i><span><b>{label}</b><small>{modelNotes[model]}</small></span></button>; })}</div><small className="review-model-battle-hint">目前：{values.map((model) => modelLabels[model]).join("＋")}</small></div>;
}
...
function ReviewHistoryPanel({ history, onRestart }: { history: ReviewHistoryEntry[]; onRestart: () => void }) {
  if (!history.length) return <section className="review-history-panel"><div className="review-history-head"><div><span>REVIEW HISTORY</span><h3>歷次對戰</h3></div><p>完成一場對話後，系統會在這裡保留使用的模型與比較結果。</p></div><div className="review-history-empty">這一題還沒有作答紀錄。完成第一次對話後，下一次可以直接重做。</div></section>;
  return <section className="review-history-panel"><div className="review-history-head"><div><span>REVIEW HISTORY</span><h3>歷次對戰紀錄</h3></div><button type="button" onClick={onRestart}>同題重新作答 <b>＋</b></button></div><div className="review-history-list">{history.map((entry) => { const saved = (() => { try { return JSON.parse(entry.resultJson) as { final?: ReviewResult; rounds?: ReviewResult[] }; } catch { return {}; } })(); const finalResult = saved.final; return <details className="review-history-item" key={entry.id}><summary><span><b>第 {entry.attemptNumber} 次</b><small>{entry.year}｜{entry.subject}｜第{entry.questionNumber}題　{new Date(entry.createdAt).toLocaleString("zh-TW", { hour12: false })}</small></span><strong>{entry.scholarModels.join("＋") || "同學作答"}</strong></summary><div className="review-history-body"><div className="review-history-meta"><span>老師：{entry.teacherModel}</span><span>學霸：{entry.scholarModels.join("＋") || "同學"}</span><span>點評：{entry.commentatorModel}</span><span>{entry.inputTokens + entry.outputTokens} tokens｜{Math.round(entry.durationMs / 100) / 10} 秒</span></div>{finalResult ? <><ModelComparisonTable result={finalResult} compact /><div className="review-history-verdict"><b>固定點評</b><p>{cleanReviewText(finalResult.commentator?.text ?? finalResult.commentatorError ?? "尚無點評內容")}</p></div></> : <p className="review-history-empty">本次紀錄只有模型設定，對答內容尚未保存。</p>}</div></details>; })}</div></section>;
}
...
function DebatePanel({ question, result, running, mode, countdownSeconds, participantMode, teacherModel, completedRounds, onNextStage, onGoQuestion, onRestart, onComplete }: { question: Question | null; result: ReviewResult | null; running: boolean; mode: DebateMode; countdownSeconds: number; participantMode: ParticipantMode; teacherModel: Provider; completedRounds: ReviewResult[]; onNextStage: (result: ReviewResult) => void; onGoQuestion: () => void; onRestart: () => void; onComplete: (result: ReviewResult, rounds: ReviewResult[]) => void }) {
  ...
  function moveNext() {
    ...
    else if (phase === "scholar-reply") setPhase(nextArgumentStage(result?.argumentStage ?? "major-premise") ? "transition-stage" : "transition-verdict");
  }
  ...
  const title = phase === "teacher-question" ? "老師先鎖定一個爭點" : phase === "scholar-answer" ? "學霸針對爭點回答" : phase === "teacher-follow-up" ? "老師沿著同一爭點抓漏洞" : phase === "scholar-reply" ? "學霸補足爭點的規範與涵攝" : phase === "transition-stage" ? `${argumentStageLabels[currentStage].short}完成` : phase === "verdict" ? "固定點評人的爭點判斷" : "點評人串場中";
  const notice = phase === "teacher-question" ? `${argumentStageLabels[currentStage].full}。老師先說清楚這一回合的爭點，再提出一個問題。` : phase === "scholar-answer" ? `學霸必須針對${argumentStageLabels[currentStage].short}，帶入題目事實回答。` : phase === "teacher-follow-up" ? "老師不換題，沿著同一個爭點把論證斷點問清楚。" : phase === "scholar-reply" ? "學霸要正面回應原爭點，補上規範與個案涵攝。" : phase === "transition-stage" ? followingStage ? `接下來不是結束，請按下方按鈕進入${argumentStageLabels[followingStage].short}。` : "三段論法已完成，接下來由固定點評人整理。" : phase === "transition-verdict" ? "三段論法完成，接下來由固定點評人整理這一回合的爭點。" : "先確認本回合真正的法律爭點，再整理漏洞與考場寫法。";
...
function StudentScholarPanel({ question, result, running, teacherModel, onGoQuestion, onRestart, onComplete }: { question: Question | null; result: ReviewResult | null; running: boolean; teacherModel: Provider; onGoQuestion: () => void; onRestart: () => void; onComplete: (result: ReviewResult, rounds: ReviewResult[]) => void }) {
  ...
  return <section className="review-conversation-stage student-scholar-stage"><DialogueQuestion question={dialogueQuestion} /><CompletedStageHistory rounds={completedRounds} /><div className="review-focus-note"><span>同學扮演學霸</span><p>{argumentStageLabels[currentStage].full}。不限時間；每一段送出後，畫面會接續下一段，不會停在爭點結束。</p></div><div className="review-round-label"><span>{argumentStageLabels[currentStage].short}｜你來回答</span><i /><strong className="review-unlimited-badge">不限時間作答</strong></div><div className="review-conversation-head"><div><span className="review-conversation-live"><i />STUDENT PRACTICE</span><h3>{isVerdict ? "固定點評人的爭點判斷" : isTransition ? `${argumentStageLabels[currentStage].short}已完成` : stage === "teacher-question" ? "老師先鎖定一個爭點" : stage === "teacher-follow-up" ? "老師沿著同一爭點追問" : "你正在扮演學霸"}</h3><p>{isVerdict ? "現在由固定點評人整理你的回答與修正方向。" : isTransition ? followingStage ? `接下來進入${argumentStageLabels[followingStage].short}。前一段內容已保留。` : "三段論法已完成，準備進入固定點評。" : "沒有倒數、沒有自動送出；可以慢慢想清楚再回答。"}</p></div><div className="review-unlimited-clock" aria-label="不限時間"><b>∞</b><small>不限時</small></div></div><div className="review-chat-thread"><div className="review-judge-line"><span className="review-judge-badge">S</span><p><b>點評人 Sol</b>{isVerdict ? "你的三段回答已完成，現在整理爭點、漏洞與考場寫法。" : isTransition ? "這一段已經完成，但整個爭點還沒有結束；請按下方按鈕繼續。" : "我只負責最後點評；本階段由你親自回答，不會替你先寫答案。"}</p></div><ArgumentBubble side="positive" label={liveResult.models.teacher} title="老師｜先問本段爭點" text={teacherQuestion} /><div className="student-answer-box"><div className="student-answer-box-head"><span>我｜扮演學霸</span><small>{argumentStageLabels[currentStage].short}：回答老師剛才的問題</small></div>{stage === "teacher-question" ? <><textarea value={answer} onChange={(event) => setAnswer(event.target.value)} rows={7} placeholder={argumentStageLabels[currentStage].prompt} disabled={busy} /><button type="button" className="student-submit-button" onClick={() => void submitAnswer()} disabled={busy || !answer.trim()}>{busy ? "老師整理追問中…" : "送出回答，請老師追問"}<span>→</span></button></> : <div className="student-answer-preview"><p>{answerText}</p><small>已送出，內容可在下方回看</small></div>}</div>{stage !== "teacher-question" && <><ArgumentBubble side="positive" label={liveResult.models.teacher} title="老師｜追問同一爭點" text={teacherFollowUp} /><div className="student-answer-box"><div className="student-answer-box-head"><span>我｜扮演學霸</span><small>回應追問，補足規範與涵攝</small></div>{stage === "teacher-follow-up" ? <><textarea value={reply} onChange={(event) => setReply(event.target.value)} rows={7} placeholder="請回應老師追問，修正前一段不足，並完成這一段的答題。" disabled={busy} /><button type="button" className="student-submit-button" onClick={() => void submitReply()} disabled={busy || !reply.trim()}>{busy ? "處理中…" : followingStage ? `送出本段，進入${argumentStageLabels[followingStage].short}` : "送出學霸回應，請 Sol 點評"}<span>→</span></button></> : <div className="student-answer-preview"><p>{replyText}</p><small>已送出，內容可在下方回看</small></div>}</div></>}{isTransition && followingStage && <div className="review-stage-transition"><span className="review-pulse" /><div><b>{argumentStageLabels[currentStage].short}已完成</b><p>現在把同一個法律爭點帶入{argumentStageLabels[followingStage].short}，不是重新選題。</p></div><button className="review-continue-button" onClick={() => void advanceStudentStage()} disabled={busy}>{busy ? "正在準備下一段…" : `進入${argumentStageLabels[followingStage].short}`}<span>→</span></button></div>}{isVerdict && <div className="review-commentator-card featured" id="expert"><div className="review-commentator-head"><span className="review-judge-badge">S</span><div><b>固定 AI 點評人</b><strong>{liveResult.models.commentator}</strong></div><span className="review-expert-tag">POINT OF VIEW</span></div>{liveResult.commentator?.text ? <p>{cleanReviewText(liveResult.commentator.text)}</p> : <div className="review-commentator-empty">{cleanReviewText(liveResult.commentatorError || "固定點評暫時沒有可顯示內容。")}</div>}</div>}{error && <p className="student-practice-error" role="alert">{error}</p>}</div>{isVerdict && <div className="review-jump-actions"><button className="selected" onClick={() => document.getElementById("student-feedback")?.scrollIntoView({ behavior: "smooth" })}>看完點評，留下你的判斷</button><button onClick={onRestart}>同題重新作答</button></div>}</section>;
}
