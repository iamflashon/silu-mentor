  ...
  return <main className="review-shell">
    ...
    <section className="review-workspace" id="battle">
      ...
      <div className="review-main"><div className="review-main-head"><div><p>{questionMeta}</p><h2>{activePanel === "question" ? "先讀題目，再安排對話" : activePanel === "debate" ? "老師問・學霸答" : "固定點評人的判斷"}</h2></div><span className="review-confidential">資料依據 <b>{question?.hasTeacherAnswer ? "已連結老師擬答" : "題目資料已連結"}</b></span></div>
        ...
        {activePanel !== "question" && <DebatePanel question={question} result={result} running={running} panel={activePanel} mode={debateMode} countdownSeconds={countdownSeconds} onGoQuestion={() => setActivePanel("question")} onComplete={() => setDebateCompleted(true)} />}
...
function DebatePanel({ question, result, running, panel, mode, countdownSeconds, onGoQuestion, onComplete }: { question: Question | null; result: ReviewResult | null; running: boolean; panel: "debate" | "verdict"; mode: DebateMode; countdownSeconds: number; onGoQuestion: () => void; onComplete: () => void }) {
