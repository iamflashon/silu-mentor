...
type ReviewResult = { question: Question; models: { teacher: string; scholar: string; commentator: string }; argumentStage?: ArgumentStage; teacherQuestion: ModelRun | null; scholarAnswer: ModelRun | null; teacherFollowUp: ModelRun | null; scholarReply: ModelRun | null; teacherError?: string | null; scholarError?: string | null; commentator: ModelRun | null; commentatorError?: string; participantMode?: ParticipantMode };
...
export default function ReviewPage() {
  ...
  async function startDebate() {
    ...
    try {
      ...
      setResult({ question, argumentStage: "major-premise", models: { teacher: modelLabels[teacherModel], scholar: participantMode === "student-scholar" ? "同學（學霸角色）" : modelLabels[scholarModel], commentator: "Sol" }, teacherQuestion: null, scholarAnswer: null, teacherFollowUp: null, scholarReply: null, commentator: null, participantMode, teacherError: error instanceof Error ? error.message : "暫時無法開始" });
  ...
  async function advanceAiStage(current: ReviewResult) {
    ...
    try {
      const response = await fetch("/api/review", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
        questionId: current.question.id, teacherModel, scholarModel, participantMode: "ai-scholar", stage: "next-stage", argumentStage: targetStage,
...
function DebatePanel({ question, result, running, mode, countdownSeconds, participantMode, teacherModel, completedRounds, onNextStage, onGoQuestion, onComplete }: { question: Question | null; result: ReviewResult | null; running: boolean; mode: DebateMode; countdownSeconds: number; participantMode: ParticipantMode; teacherModel: Provider; completedRounds: ReviewResult[]; onNextStage: (result: ReviewResult) => void; onGoQuestion: () => void; onComplete: () => void }) {
