...
export default function LegalSearchLabPage() {
  ...
  return <main style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 20px 80px", color: "#172033" }}>
    ...
    {questionLibrary && <section style={{ ...card, marginBottom: 18, padding: 0, overflow: "hidden", background: "#fff" }}>
      <div style={{ padding: "18px 20px 14px", background: "linear-gradient(135deg, #f7faff 0%, #eef4fb 100%)", borderBottom: "1px solid #d9e2ee" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 14, alignItems: "flex-start", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><span style={{ padding: "6px 10px", borderRadius: 999, background: "#fff", border: "1px solid #d3deeb", color: "#42536b", fontSize: 14 }}>題庫 {questionLibrary.bankSize} 題</span><span style={{ padding: "6px 10px", borderRadius: 999, background: "#183b66", color: "#fff", fontSize: 14 }}>已保存 {questionLibrary.savedResearchRuns} 次研究</span></div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gap: 8, padding: "14px 20px 20px" }}>
        {(questionLibraryTab === "common" ? questionLibrary.common : questionLibrary.history).map((item, index) => <button type="button" key={`${questionLibraryTab}-${item.question}`} disabled={loading} onClick={() => { setQuestion(item.question); setPersona(""); setPersonaKey(""); setResult(null); }} style={{ display: "grid", gridTemplateColumns: "34px minmax(0, 1fr) auto", gap: 12, alignItems: "center", width: "100%", minWidth: 0, border: "1px solid #d8e0eb", borderRadius: 10, padding: "12px 14px", background: "#fff", color: "#243c5a", textAlign: "left", cursor: loading ? "wait" : "pointer" }}><strong style={{ display: "inline-grid", placeItems: "center", width: 30, height: 30, borderRadius: 8, background: "#e8f0fa", color: "#183b66" }}>{index + 1}</strong><span style={{ minWidth: 0, lineHeight: 1.55 }}>{item.question}</span><small style={{ padding: "4px 8px", borderRadius: 999, background: "#f0f3f7", color: "#667287", whiteSpace: "nowrap" }}>研究 {item.askedCount} 次</small></button>)}
