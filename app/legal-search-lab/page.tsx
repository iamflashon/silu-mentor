...
type Simulation = { notice: string; assessment: string; qualifiedCases: number; totalUniqueCases: number; cloudAvailableTotal: number; rounds: Array<{ round: number; purpose: string; queryRuns: Array<{ query: string; hits?: number; matched?: number }>; uniqueCasesSoFar: number }>; results: CaseItem[]; exploratoryCandidates: CaseItem[]; access?: Access; researcher?: { planner: "sol" | "rules"; model: string | null; issues?: string[]; terms?: string[]; statutes?: string[]; requiredEvidence?: string[]; exclude?: string[] }; tokenUsage?: { stages: Array<{ stage: string; model: string | null; inputTokens: number; cachedTokens: number; outputTokens: number; totalTokens: number; note: string }>; internalTotalTokens: number; externalAnswerTokens: null; externalAnswerNote: string } };
...
export default function LegalSearchLabPage() {
  ...
  return <main style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 20px 80px", color: "#172033" }}>
    ...
    {result && <>
      {result.researcher && <section style={{ marginTop: 28 }}><h2>Sol 如何拆解這個問題</h2><p style={{ color: "#59677b" }}>Sol 只制定研究策略，不生成法律答案。以下內容會實際影響資料庫查詢與證據篩選。</p><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}><PlanCard title="法律爭點" items={result.researcher.issues} empty={result.researcher.planner === "rules" ? "Sol 暫時不可用，本次採保守規則拆解" : "未提出"} /><PlanCard title="同義詞與裁判常用語" items={result.researcher.terms} /><PlanCard title="可能適用法條" items={result.researcher.statutes} /><PlanCard title="全文必須找到的證據" items={result.researcher.requiredEvidence} /><PlanCard title="必須排除的假命中" items={result.researcher.exclude} /></div></section>}
      ...
      {result.tokenUsage && <section style={{ marginTop: 30 }}><h2>本次 MCP Token 使用</h2><div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", background: "white" }}><thead><tr>{["處理階段", "模型", "輸入", "快取", "輸出", "合計"].map(label => <th key={label} style={{ textAlign: "left", padding: 10, borderBottom: "2px solid #cbd5e3" }}>{label}</th>)}</tr></thead><tbody>{result.tokenUsage.stages.map(stage => <tr key={stage.stage}><td style={{ padding: 10, borderBottom: "1px solid #e2e7ef" }}>{stage.stage}<small style={{ display: "block", color: "#6c788b" }}>{stage.note}</small></td><td style={{ padding: 10, borderBottom: "1px solid #e2e7ef" }}>{stage.model || "—"}</td><td style={{ padding: 10, borderBottom: "1px solid #e2e7ef" }}>{stage.inputTokens.toLocaleString()}</td><td style={{ padding: 10, borderBottom: "1px solid #e2e7ef" }}>{stage.cachedTokens.toLocaleString()}</td><td style={{ padding: 10, borderBottom: "1px solid #e2e7ef" }}>{stage.outputTokens.toLocaleString()}</td><td style={{ padding: 10, borderBottom: "1px solid #e2e7ef", fontWeight: 700 }}>{stage.totalTokens.toLocaleString()}</td></tr>)}</tbody></table></div><p style={{ background: "#eef4fa", borderRadius: 9, padding: 12 }}><strong>MCP 內部合計：{result.tokenUsage.internalTotalTokens.toLocaleString()} Tokens。</strong><br />{result.tokenUsage.externalAnswerNote}</p></section>}
...
function PlanCard({ title, items, empty = "未提出" }: { title: string; items?: string[]; empty?: string }) {
  return <article style={{ ...card, background: "#fbfcfe" }}><strong style={{ color: "#183b66" }}>{title}</strong>{items?.length ? <ul style={{ margin: "10px 0 0", paddingLeft: 20, lineHeight: 1.7 }}>{items.map(item => <li key={item}>{item}</li>)}</ul> : <p style={{ margin: "10px 0 0", color: "#7a8698" }}>{empty}</p>}</article>;
}
