...
第一階段建立安全心跳與硬體／模型狀態回報。第二、三階段加入受限工作佇列與 GPU OCR。v0.6.7 可對已完成影片單獨補做 Whisper 字幕與 AI 摘要；若本機已有 `transcript.srt`，會略過重新辨識並直接補傳。字幕模組異常時不會關閉主節點或阻塞下一支影片。原始 PDF、Word 與影片都留在公司電腦，雲端只接收文字索引或播放所需的 HLS 切片、清單、縮圖與字幕。
