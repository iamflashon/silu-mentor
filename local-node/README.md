...
第一階段建立安全心跳與硬體／模型狀態回報。第二、三階段加入受限工作佇列與 GPU OCR。v0.6.6 可對已完成影片單獨補做 Whisper 字幕與 AI 摘要，不會重新轉檔或上傳既有 HLS；字幕模組異常時也不會關閉主節點或阻塞下一支影片。原始 PDF、Word 與影片都留在公司電腦，雲端只接收文字索引或播放所需的 HLS 切片、清單、縮圖與字幕。
