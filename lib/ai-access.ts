...
export function publicAiAccess(row: typeof aiAccessEntitlements.$inferSelect | null) {
  if (!row) return { active:false, quotaTotal:0, quotaUsed:0, remaining:0, startsAt:null, expiresAt:null };
  return { active:true, quotaTotal:row.quotaTotal, quotaUsed:row.quotaUsed, remaining:Math.max(0,row.quotaTotal-row.quotaUsed), startsAt:row.startsAt.toISOString(), expiresAt:row.expiresAt.toISOString(), source:row.source };
}
